...
pub trait GeneralOps {
    fn execute_x0(&mut self, op: OpParams) -> u8;
    fn execute_x1(&mut self, op: OpParams) -> u8;
    fn execute_x2(&mut self, op: OpParams) -> u8;
    fn execute_x3(&mut self, op: OpParams) -> u8;
}
...
impl<M: MemoryInterface, I: IoInterface> GeneralOps for Z80<M, I> {
    fn execute_x0(&mut self, op: OpParams) -> u8 {
        dispatch_z!(
            op.z,
            execute_x0_control_misc(self, op.y),
            execute_x0_load_add_hl(self, op.y),
            execute_x0_load_indirect(self, op.y),
            execute_x0_inc_dec_rp(self, op.y),
            execute_x0_inc_r(self, op.y),
            execute_x0_dec_r(self, op.y),
            execute_x0_ld_r_n(self, op.y),
            execute_x0_rotate_accum_flags(self, op.y)
        )
    }
    ...
    fn execute_x1(&mut self, op: OpParams) -> u8 {
        if op.y == 6 && op.z == 6 {
            ...
            // LD r, r'
            let val = self.get_reg(op.z);
            self.set_reg(op.y, val);
            if op.y == 6 || op.z == 6 {
                7
            } else {
                4
    ...
    fn execute_x3(&mut self, op: OpParams) -> u8 {
        dispatch_z!(
            op.z,
            execute_x3_ret_cc(self, op.y),
            execute_x3_pop_ret_exx(self, op.y),
            execute_x3_jp_cc(self, op.y),
            execute_x3_jp_out_ex_di_ei(self, op.y),
            execute_x3_call_cc(self, op.y),
            execute_x3_push_call_prefixes(self, op.y),
            execute_x3_alu_n(self, op.y),
            execute_x3_rst(self, op.y)
        )
    }
}
