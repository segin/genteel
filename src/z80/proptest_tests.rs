...
proptest! {
    ...
    fn prop_bc_roundtrip(val in 0u16..=0xFFFF) {
        let (mut z80, mut bus) = create_z80_with_program(&[]);
        z80.set_bc(val);
    ...
    fn prop_ld_bc_nn(low in 0u8..=255, high in 0u8..=255) {
        let (mut z80, mut bus) = create_z80_with_program(&[0x01, low, high]);
        z80.step(&mut bus);
    ...
    fn prop_ld_de_nn(low in 0u8..=255, high in 0u8..=255) {
        let (mut z80, mut bus) = create_z80_with_program(&[0x11, low, high]);
        z80.step(&mut bus);
        let expected = ((high as u16) << 8) | (low as u16);
    ...
    fn prop_ld_hl_nn(low in 0u8..=255, high in 0u8..=255) {
        let (mut z80, mut bus) = create_z80_with_program(&[0x21, low, high]);
        z80.step(&mut bus);
        let expected = ((high as u16) << 8) | (low as u16);
    ...
    fn prop_ld_sp_nn(low in 0u8..=255, high in 0u8..=255) {
        let (mut z80, mut bus) = create_z80_with_program(&[0x31, low, high]);
        z80.step(&mut bus);
        let expected = ((high as u16) << 8) | (low as u16);
    ...
    fn prop_inc_bc_adds_one(val in 0u16..=0xFFFF) {
        let (mut z80, mut bus) = create_z80_with_program(&[0x03]);
        z80.set_bc(val);
        z80.step(&mut bus);
    ...
    fn prop_dec_bc_subtracts_one(val in 0u16..=0xFFFF) {
        let (mut z80, mut bus) = create_z80_with_program(&[0x0B]);
        z80.set_bc(val);
        z80.step(&mut bus);
    ...
    fn prop_inc_de_adds_one(val in 0u16..=0xFFFF) {
        let (mut z80, mut bus) = create_z80_with_program(&[0x13]);
        z80.set_de(val);
        z80.step(&mut bus);
    ...
    fn prop_inc_hl_adds_one(val in 0u16..=0xFFFF) {
        let (mut z80, mut bus) = create_z80_with_program(&[0x23]);
        z80.set_hl(val);
        z80.step(&mut bus);
    ...
    fn prop_inc_sp_adds_one(val in 0u16..=0xFFFF) {
        let (mut z80, mut bus) = create_z80_with_program(&[0x33]);
        z80.sp = val;
        z80.step(&mut bus);
    ...
    fn prop_inc_a(val in 0u8..=255) {
        let (mut z80, mut bus) = create_z80_with_program(&[0x3C]);
        z80.a = val;
        z80.step(&mut bus);
    ...
    fn prop_dec_a(val in 0u8..=255) {
        let (mut z80, mut bus) = create_z80_with_program(&[0x3D]);
        z80.a = val;
        z80.step(&mut bus);
    ...
    fn prop_inc_b(val in 0u8..=255) {
        let (mut z80, mut bus) = create_z80_with_program(&[0x04]);
        z80.b = val;
        z80.step(&mut bus);
    ...
    fn prop_add_a_b(a in 0u8..=255, b in 0u8..=255) {
        let (mut z80, mut bus) = create_z80_with_program(&[0x80]);
        z80.a = a;
        z80.b = b;
        z80.step(&mut bus);
    ...
    fn prop_sub_a_b(a in 0u8..=255, b in 0u8..=255) {
        let (mut z80, mut bus) = create_z80_with_program(&[0x90]);
        z80.a = a;
        z80.b = b;
        z80.step(&mut bus);
    ...
    fn prop_and_a_b(a in 0u8..=255, b in 0u8..=255) {
        let (mut z80, mut bus) = create_z80_with_program(&[0xA0]);
        z80.a = a;
        z80.b = b;
        z80.step(&mut bus);
    ...
    fn prop_or_a_b(a in 0u8..=255, b in 0u8..=255) {
        let (mut z80, mut bus) = create_z80_with_program(&[0xB0]);
        z80.a = a;
        z80.b = b;
        z80.step(&mut bus);
        prop_assert_eq!(z80.a, a | b);
    ...
    fn prop_xor_a_b(a in 0u8..=255, b in 0u8..=255) {
        let (mut z80, mut bus) = create_z80_with_program(&[0xA8]);
        z80.a = a;
        z80.b = b;
        z80.step(&mut bus);
        prop_assert_eq!(z80.a, a ^ b);
    ...
    fn prop_cp_does_not_modify_a(a in 0u8..=255, b in 0u8..=255) {
        let (mut z80, mut bus) = create_z80_with_program(&[0xB8]);
        z80.a = a;
        z80.b = b;
        z80.step(&mut bus);
    ...
    fn prop_nop_only_advances_pc(a in 0u8..=255, b in 0u8..=255, c in 0u8..=255) {
        let (mut z80, mut bus) = create_z80_with_program(&[0x00]);
        z80.a = a;
        z80.b = b;
        z80.c = c;
        let old_sp = z80.sp;
    ...
    fn prop_ld_b_c_copies(val in 0u8..=255) {
        let (mut z80, mut bus) = create_z80_with_program(&[0x41]);
        z80.c = val;
        z80.step(&mut bus);
    ...
    fn prop_ld_a_b_copies(val in 0u8..=255) {
        let (mut z80, mut bus) = create_z80_with_program(&[0x78]);
        z80.b = val;
        z80.step(&mut bus);
    ...
    fn prop_rlca_carry_is_bit_7(val in 0u8..=255) {
        let (mut z80, mut bus) = create_z80_with_program(&[0x07]);
        z80.a = val;
        z80.step(&mut bus);
    ...
    fn prop_rrca_carry_is_bit_0(val in 0u8..=255) {
        let (mut z80, mut bus) = create_z80_with_program(&[0x0F]);
        z80.a = val;
        z80.step(&mut bus);
    ...
    fn prop_push_pop_bc_roundtrip(val in 0u16..=0xFFFF) {
        let (mut z80, mut bus) = create_z80_with_program(&[0xC5, 0xC1]); // PUSH BC, POP BC
        z80.sp = 0x8000;
        z80.set_bc(val);
        z80.step(&mut bus); // PUSH
        z80.set_bc(0x0000);
        z80.step(&mut bus); // POP
        prop_assert_eq!(z80.bc(), val);
    ...
    fn prop_push_pop_af_roundtrip(val in 0u16..=0xFFFF) {
        let (mut z80, mut bus) = create_z80_with_program(&[0xF5, 0xF1]); // PUSH AF, POP AF
        z80.sp = 0x8000;
        z80.set_af(val);
        z80.step(&mut bus); // PUSH
        z80.set_af(0x0000);
    ...
    fn prop_cb_set_bit(bit in 0u8..8, val in 0u8..=255) {
        ...
        let (mut z80, mut bus) = create_z80_with_program(&[0xCB, opcode | 0x07]);
        z80.a = val;
        z80.step(&mut bus);
    ...
    fn prop_ld_ix_nn(low in 0u8..=255, high in 0u8..=255) {
        let (mut z80, mut bus) = create_z80_with_program(&[0xDD, 0x21, low, high]);
        z80.step(&mut bus);
        let expected = ((high as u16) << 8) | (low as u16);
    ...
    fn prop_ld_iy_nn(low in 0u8..=255, high in 0u8..=255) {
        let (mut z80, mut bus) = create_z80_with_program(&[0xFD, 0x21, low, high]);
        z80.step(&mut bus);
        let expected = ((high as u16) << 8) | (low as u16);
    ...
    fn prop_inc_ix(val in 0u16..=0xFFFF) {
        let (mut z80, mut bus) = create_z80_with_program(&[0xDD, 0x23]);
        z80.ix = val;
        z80.step(&mut bus);
    ...
    fn prop_dec_iy(val in 0u16..=0xFFFF) {
        let (mut z80, mut bus) = create_z80_with_program(&[0xFD, 0x2B]);
        z80.iy = val;
        z80.step(&mut bus);
