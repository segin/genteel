...
impl<M: MemoryInterface, I: IoInterface> Z80<M, I> {
    ...
    fn execute_index_prefix(&mut self, is_ix: bool) -> u8 {
        ...
        match opcode {
            // 16-bit Arithmetic / Load / Stack
            0x09 | 0x19 | 0x21 | 0x22 | 0x23 | 0x29 | 0x2A | 0x2B | 0x39 | 0xE1 | 0xE5 | 0xF9 => {
                self.execute_index_16bit(opcode, is_ix)
            }

            // 8-bit Index High/Low (Documented)
            0x24 | 0x25 | 0x26 | 0x2C | 0x2D | 0x2E => {
                self.execute_index_8bit_immediate(opcode, is_ix)
            }

            // Indirect (IX+d) - Documented
            // INC/DEC/LD (IX+d), n
            0x34 | 0x35 | 0x36 |
            // LD r, (IX+d)
            ...
            0x86 | 0x8E | 0x96 | 0x9E | 0xA6 | 0xAE | 0xB6 | 0xBE => {
                self.execute_index_indirect(opcode, is_ix)
            }

            // Control / Exchange
            0xE3 | 0xE9 => self.execute_index_control(opcode, is_ix),

            // CB Prefix
            0xCB => self.execute_index_cb_setup(is_ix),
            ...
            // Undocumented 8-bit Halves (Ranges)
            0x40..=0x7F => self.execute_index_8bit_halves_ld(opcode, is_ix),
            0x80..=0xBF => self.execute_index_8bit_halves_alu(opcode, is_ix),

            _ => 8, // NOP
        }
    }

    fn execute_index_16bit(&mut self, opcode: u8, is_ix: bool) -> u8 {
        match opcode {
            0x09 => {
                let val = self.bc();
    ...
    fn execute_index_8bit_immediate(&mut self, opcode: u8, is_ix: bool) -> u8 {
        match opcode {
            0x24 => {
    ...
    fn execute_index_indirect(&mut self, opcode: u8, is_ix: bool) -> u8 {
        let d = self.fetch_byte() as i8;
    ...
    fn execute_index_control(&mut self, opcode: u8, is_ix: bool) -> u8 {
        match opcode {
            0xE3 => {
                let val = self.read_word(self.sp);
                let idx = self.get_index_val(is_ix);
    ...
    fn execute_index_cb_setup(&mut self, is_ix: bool) -> u8 {
        let d = self.fetch_byte() as i8;
        let addr = self.calc_index_addr(d, is_ix);
        let opcode = self.fetch_byte();
    ...
    fn execute_index_8bit_halves_ld(&mut self, opcode: u8, is_ix: bool) -> u8 {
        if opcode == 0x76 {
            self.halted = true;
            return 8;
        }
    ...
    fn execute_index_8bit_halves_alu(&mut self, opcode: u8, is_ix: bool) -> u8 {
