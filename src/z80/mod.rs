...
impl<M: MemoryInterface, I: IoInterface> Z80<M, I> {
    ...
    fn execute_index_prefix(&mut self, is_ix: bool) -> u8 {
        ...
        match opcode {
            ...
            // 16-bit Load/Store
            0x21 | 0x22 | 0x2A => self.execute_index_ld_16(opcode, is_ix),

            // 16-bit Inc/Dec
            0x23 | 0x2B => self.execute_index_inc_dec_16(opcode, is_ix),

            // 8-bit Halves (IXH/IXL) direct ops
            0x24 | 0x25 | 0x26 | 0x2C | 0x2D | 0x2E => self.execute_index_8bit_halves(opcode, is_ix),

            // 8-bit Memory Displacement Ops (INC/DEC/LD (IX+d), n)
            0x34 | 0x35 | 0x36 => self.execute_index_mem_8bit(opcode, is_ix),

            // Specific ALU (IX+d)
            0x86 | 0x8E | 0x96 | 0x9E | 0xA6 | 0xAE | 0xB6 | 0xBE => self.execute_index_alu_disp(opcode, is_ix),

            // LD r, (IX+d)
            0x46 | 0x4E | 0x56 | 0x5E | 0x66 | 0x6E | 0x7E => self.execute_index_ld_from_disp(opcode, is_ix),

            // LD (IX+d), r
            0x70..=0x75 | 0x77 => self.execute_index_ld_to_disp(opcode, is_ix),
            ...
            0x40..=0x7F => self.execute_index_undoc_ld(opcode, is_ix),
    ...
    fn execute_index_ld_16(&mut self, opcode: u8, is_ix: bool) -> u8 {
        match opcode {
            0x21 => {
    ...
    fn execute_index_alu_disp(&mut self, opcode: u8, is_ix: bool) -> u8 {
        let d = self.fetch_byte() as i8;
        let addr = self.calc_index_addr(d, is_ix);
        let val = self.read_byte(addr);
    ...
    fn execute_index_ld_from_disp(&mut self, opcode: u8, is_ix: bool) -> u8 {
        let d = self.fetch_byte() as i8;
        let addr = self.calc_index_addr(d, is_ix);
        let val = self.read_byte(addr);
        let r = (opcode >> 3) & 0x07;
    ...
    fn execute_index_ld_to_disp(&mut self, opcode: u8, is_ix: bool) -> u8 {
        let d = self.fetch_byte() as i8;
        let addr = self.calc_index_addr(d, is_ix);
        let r = opcode & 0x07;
    ...
    fn execute_index_undoc_ld(&mut self, opcode: u8, is_ix: bool) -> u8 {
