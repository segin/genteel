...
fn timing_jr_nc_taken() {
    let mut c = z80(&[0x30, 0x00]);
    c.f = 0;
    assert_eq!(c.step(), 12);
}
#[test]
fn timing_jr_c_taken() {
    let mut c = z80(&[0x38, 0x00]);
...
#[test]
fn timing_cb_rlc_b() {
    let mut c = z80(&[0xCB, 0x00]);
    assert_eq!(c.step(), 8);
}
#[test]
fn timing_cb_rlc_hl() {
    let mut c = z80(&[0xCB, 0x06]);
    c.set_hl(0x100);
    assert_eq!(c.step(), 15);
}
#[test]
fn timing_cb_bit_0_b() {
    let mut c = z80(&[0xCB, 0x40]);
    assert_eq!(c.step(), 8);
}
#[test]
...
fn timing_cb_res_0_b() {
    let mut c = z80(&[0xCB, 0x80]);
    assert_eq!(c.step(), 8);
}
#[test]
fn timing_cb_res_0_hl() {
    let mut c = z80(&[0xCB, 0x86]);
    c.set_hl(0x100);
    assert_eq!(c.step(), 15);
}
#[test]
fn timing_cb_set_0_b() {
    let mut c = z80(&[0xCB, 0xC0]);
    assert_eq!(c.step(), 8);
}
#[test]
fn timing_cb_set_0_hl() {
    let mut c = z80(&[0xCB, 0xC6]);
    c.set_hl(0x100);
    assert_eq!(c.step(), 15);
}
...
#[test]
fn timing_ed_in_b_c() {
    let mut c = z80(&[0xED, 0x40]);
    assert_eq!(c.step(), 12);
}
#[test]
fn timing_ed_out_c_b() {
    let mut c = z80(&[0xED, 0x41]);
    assert_eq!(c.step(), 12);
}
#[test]
...
fn timing_ed_ld_nn_bc() {
    let mut c = z80(&[0xED, 0x43, 0x00, 0x10]);
    assert_eq!(c.step(), 20);
}
#[test]
fn timing_ed_neg() {
...
fn timing_ed_ld_bc_nn() {
    let mut c = z80(&[0xED, 0x4B, 0x00, 0x10]);
    assert_eq!(c.step(), 20);
}
#[test]
fn timing_ed_reti() {
...
fn timing_dd_ld_ix_nn() {
    let mut c = z80(&[0xDD, 0x21, 0x00, 0x00]);
    assert_eq!(c.step(), 14);
}
#[test]
fn timing_dd_ld_nn_ix() {
    let mut c = z80(&[0xDD, 0x22, 0x00, 0x10]);
    assert_eq!(c.step(), 20);
}
#[test]
fn timing_dd_inc_ix() {
...
fn timing_dd_ld_ix_nn_ind() {
    let mut c = z80(&[0xDD, 0x2A, 0x00, 0x10]);
    assert_eq!(c.step(), 20);
}
#[test]
fn timing_dd_dec_ix() {
...
fn timing_dd_inc_ix_d() {
    let mut c = z80(&[0xDD, 0x34, 0x00]);
    c.ix = 0x100;
    assert_eq!(c.step(), 23);
}
#[test]
fn timing_dd_dec_ix_d() {
    let mut c = z80(&[0xDD, 0x35, 0x00]);
    c.ix = 0x100;
    assert_eq!(c.step(), 23);
}
#[test]
fn timing_dd_ld_ix_d_n() {
    let mut c = z80(&[0xDD, 0x36, 0x00, 0x00]);
    c.ix = 0x100;
    assert_eq!(c.step(), 19);
}
#[test]
fn timing_dd_add_ix_bc() {
    let mut c = z80(&[0xDD, 0x09]);
    assert_eq!(c.step(), 15);
}
#[test]
fn timing_dd_ld_b_ix_d() {
    let mut c = z80(&[0xDD, 0x46, 0x00]);
    c.ix = 0x100;
    assert_eq!(c.step(), 19);
}
#[test]
fn timing_dd_ld_ix_d_b() {
    let mut c = z80(&[0xDD, 0x70, 0x00]);
    c.ix = 0x100;
    assert_eq!(c.step(), 19);
}
#[test]
fn timing_dd_add_a_ix_d() {
    let mut c = z80(&[0xDD, 0x86, 0x00]);
    c.ix = 0x100;
    assert_eq!(c.step(), 19);
}
#[test]
...
#[test]
fn timing_ddcb_rlc() {
    let mut c = z80(&[0xDD, 0xCB, 0x00, 0x06]);
    c.ix = 0x100;
    assert_eq!(c.step(), 23);
}
#[test]
fn timing_ddcb_bit() {
    let mut c = z80(&[0xDD, 0xCB, 0x00, 0x46]);
    c.ix = 0x100;
    assert_eq!(c.step(), 20);
}
#[test]
fn timing_ddcb_res() {
    let mut c = z80(&[0xDD, 0xCB, 0x00, 0x86]);
    c.ix = 0x100;
    assert_eq!(c.step(), 23);
}
#[test]
fn timing_ddcb_set() {
    let mut c = z80(&[0xDD, 0xCB, 0x00, 0xC6]);
    c.ix = 0x100;
    assert_eq!(c.step(), 23);
}
...
fn timing_fd_ld_iy_nn() {
    let mut c = z80(&[0xFD, 0x21, 0x00, 0x00]);
    assert_eq!(c.step(), 14);
}
#[test]
fn timing_fd_ld_nn_iy() {
    let mut c = z80(&[0xFD, 0x22, 0x00, 0x10]);
    assert_eq!(c.step(), 20);
}
#[test]
fn timing_fd_inc_iy() {
...
fn timing_fd_ld_iy_nn_ind() {
    let mut c = z80(&[0xFD, 0x2A, 0x00, 0x10]);
    assert_eq!(c.step(), 20);
}
#[test]
fn timing_fd_dec_iy() {
...
fn timing_fd_inc_iy_d() {
    let mut c = z80(&[0xFD, 0x34, 0x00]);
    c.iy = 0x100;
    assert_eq!(c.step(), 23);
}
#[test]
fn timing_fd_dec_iy_d() {
    let mut c = z80(&[0xFD, 0x35, 0x00]);
    c.iy = 0x100;
    assert_eq!(c.step(), 23);
}
#[test]
fn timing_fd_ld_iy_d_n() {
    let mut c = z80(&[0xFD, 0x36, 0x00, 0x00]);
    c.iy = 0x100;
    assert_eq!(c.step(), 19);
}
#[test]
fn timing_fd_add_iy_bc() {
    let mut c = z80(&[0xFD, 0x09]);
    assert_eq!(c.step(), 15);
}
#[test]
fn timing_fd_ld_b_iy_d() {
    let mut c = z80(&[0xFD, 0x46, 0x00]);
    c.iy = 0x100;
    assert_eq!(c.step(), 19);
}
#[test]
fn timing_fd_ld_iy_d_b() {
    let mut c = z80(&[0xFD, 0x70, 0x00]);
    c.iy = 0x100;
    assert_eq!(c.step(), 19);
}
#[test]
fn timing_fd_add_a_iy_d() {
    let mut c = z80(&[0xFD, 0x86, 0x00]);
    c.iy = 0x100;
    assert_eq!(c.step(), 19);
}
#[test]
...
#[test]
fn timing_fdcb_rlc() {
    let mut c = z80(&[0xFD, 0xCB, 0x00, 0x06]);
    c.iy = 0x100;
    assert_eq!(c.step(), 23);
}
#[test]
fn timing_fdcb_bit() {
    let mut c = z80(&[0xFD, 0xCB, 0x00, 0x46]);
    c.iy = 0x100;
    assert_eq!(c.step(), 20);
}
#[test]
fn timing_fdcb_res() {
    let mut c = z80(&[0xFD, 0xCB, 0x00, 0x86]);
    c.iy = 0x100;
    assert_eq!(c.step(), 23);
}
#[test]
fn timing_fdcb_set() {
    let mut c = z80(&[0xFD, 0xCB, 0x00, 0xC6]);
    c.iy = 0x100;
    assert_eq!(c.step(), 23);
}
