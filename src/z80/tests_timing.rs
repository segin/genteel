...
fn timing_jr_z_taken() {
    let mut c = create_z80(&[0x28, 0x00]);
    c.set_flag(flags::ZERO, true);
    assert_eq!(c.step(), 12);
}
#[test]
...
fn timing_jr_nc_taken() {
    let mut c = create_z80(&[0x30, 0x00]);
    c.f = 0;
    assert_eq!(c.step(), 12);
}
#[test]
fn timing_jr_c_taken() {
    let mut c = create_z80(&[0x38, 0x00]);
...
#[test]
fn timing_ld_nn_hl() {
    let mut c = create_z80(&[0x22, 0x00, 0x10]);
    assert_eq!(c.step(), 16);
}
#[test]
fn timing_inc_hl() {
...
fn timing_ld_hl_nn_ind() {
    let mut c = create_z80(&[0x2A, 0x00, 0x10]);
    assert_eq!(c.step(), 16);
}
#[test]
fn timing_cpl() {
...
#[test]
fn timing_ld_nn_a() {
    let mut c = create_z80(&[0x32, 0x00, 0x10]);
    assert_eq!(c.step(), 13);
}
#[test]
fn timing_inc_sp() {
...
#[test]
fn timing_ld_a_nn() {
    let mut c = create_z80(&[0x3A, 0x00, 0x10]);
    assert_eq!(c.step(), 13);
}
#[test]
fn timing_ccf() {
...
fn timing_ret_nz_taken() {
    ...
    c.memory.write_byte(0x101 as u32, 0x10);
    c.f = 0;
    assert_eq!(c.step(), 11);
}
#[test]
...
#[test]
fn timing_jp_nn() {
    let mut c = create_z80(&[0xC3, 0x00, 0x10]);
    assert_eq!(c.step(), 10);
}
#[test]
...
#[test]
fn timing_add_a_n() {
    let mut c = create_z80(&[0xC6, 0x00]);
    assert_eq!(c.step(), 7);
}
#[test]
fn timing_rst_00() {
...
fn timing_cb_res_0_b() {
    let mut c = create_z80(&[0xCB, 0x80]);
    assert_eq!(c.step(), 8);
}
#[test]
fn timing_cb_res_0_hl() {
    let mut c = create_z80(&[0xCB, 0x86]);
    c.set_hl(0x100);
    assert_eq!(c.step(), 15);
}
#[test]
fn timing_cb_set_0_b() {
    let mut c = create_z80(&[0xCB, 0xC0]);
    assert_eq!(c.step(), 8);
}
#[test]
fn timing_cb_set_0_hl() {
    let mut c = create_z80(&[0xCB, 0xC6]);
    c.set_hl(0x100);
    assert_eq!(c.step(), 15);
}
...
fn timing_ed_out_c_b() {
    let mut c = create_z80(&[0xED, 0x41]);
    assert_eq!(c.step(), 12);
}
#[test]
...
fn timing_ed_ld_nn_bc() {
    let mut c = create_z80(&[0xED, 0x43, 0x00, 0x10]);
    assert_eq!(c.step(), 20);
}
#[test]
fn timing_ed_neg() {
...
fn timing_ed_ld_bc_nn() {
    let mut c = create_z80(&[0xED, 0x4B, 0x00, 0x10]);
    assert_eq!(c.step(), 20);
}
#[test]
fn timing_ed_reti() {
...
fn timing_dd_ld_nn_ix() {
    let mut c = create_z80(&[0xDD, 0x22, 0x00, 0x10]);
    assert_eq!(c.step(), 20);
}
#[test]
fn timing_dd_inc_ix() {
...
fn timing_dd_ld_ix_nn_ind() {
    let mut c = create_z80(&[0xDD, 0x2A, 0x00, 0x10]);
    assert_eq!(c.step(), 20);
}
#[test]
fn timing_dd_dec_ix() {
    let mut c = create_z80(&[0xDD, 0x2B]);
    assert_eq!(c.step(), 10);
}
#[test]
...
fn timing_dd_dec_ix_d() {
    let mut c = create_z80(&[0xDD, 0x35, 0x00]);
    c.ix = 0x100;
    assert_eq!(c.step(), 23);
}
#[test]
...
fn timing_dd_ld_b_ix_d() {
    let mut c = create_z80(&[0xDD, 0x46, 0x00]);
    c.ix = 0x100;
    assert_eq!(c.step(), 19);
}
#[test]
fn timing_dd_ld_ix_d_b() {
    let mut c = create_z80(&[0xDD, 0x70, 0x00]);
    c.ix = 0x100;
    assert_eq!(c.step(), 19);
}
#[test]
...
#[test]
fn timing_dd_pop_ix() {
    let mut c = create_z80(&[0xDD, 0xE1]);
    c.sp = 0x100;
    assert_eq!(c.step(), 14);
}
#[test]
...
fn timing_dd_push_ix() {
    let mut c = create_z80(&[0xDD, 0xE5]);
    c.sp = 0x200;
    assert_eq!(c.step(), 15);
}
#[test]
...
fn timing_dd_ld_sp_ix() {
    let mut c = create_z80(&[0xDD, 0xF9]);
    assert_eq!(c.step(), 10);
}
...
#[test]
fn timing_ddcb_res() {
    let mut c = create_z80(&[0xDD, 0xCB, 0x00, 0x86]);
    c.ix = 0x100;
    assert_eq!(c.step(), 23);
}
#[test]
...
fn timing_fd_ld_iy_nn() {
    let mut c = create_z80(&[0xFD, 0x21, 0x00, 0x00]);
    assert_eq!(c.step(), 14);
}
#[test]
fn timing_fd_ld_nn_iy() {
    let mut c = create_z80(&[0xFD, 0x22, 0x00, 0x10]);
    assert_eq!(c.step(), 20);
}
#[test]
fn timing_fd_inc_iy() {
    let mut c = create_z80(&[0xFD, 0x23]);
    assert_eq!(c.step(), 10);
}
#[test]
fn timing_fd_ld_iy_nn_ind() {
    let mut c = create_z80(&[0xFD, 0x2A, 0x00, 0x10]);
    assert_eq!(c.step(), 20);
}
#[test]
fn timing_fd_dec_iy() {
    let mut c = create_z80(&[0xFD, 0x2B]);
    assert_eq!(c.step(), 10);
}
#[test]
fn timing_fd_inc_iy_d() {
    let mut c = create_z80(&[0xFD, 0x34, 0x00]);
    c.iy = 0x100;
    assert_eq!(c.step(), 23);
}
#[test]
fn timing_fd_dec_iy_d() {
    let mut c = create_z80(&[0xFD, 0x35, 0x00]);
    c.iy = 0x100;
    assert_eq!(c.step(), 23);
}
#[test]
fn timing_fd_ld_iy_d_n() {
    let mut c = create_z80(&[0xFD, 0x36, 0x00, 0x00]);
    c.iy = 0x100;
    assert_eq!(c.step(), 19);
}
#[test]
fn timing_fd_add_iy_bc() {
    let mut c = create_z80(&[0xFD, 0x09]);
    assert_eq!(c.step(), 15);
}
#[test]
fn timing_fd_ld_b_iy_d() {
    let mut c = create_z80(&[0xFD, 0x46, 0x00]);
    c.iy = 0x100;
    assert_eq!(c.step(), 19);
}
#[test]
fn timing_fd_ld_iy_d_b() {
    let mut c = create_z80(&[0xFD, 0x70, 0x00]);
    c.iy = 0x100;
    assert_eq!(c.step(), 19);
}
#[test]
fn timing_fd_add_a_iy_d() {
    let mut c = create_z80(&[0xFD, 0x86, 0x00]);
    c.iy = 0x100;
    assert_eq!(c.step(), 19);
}
#[test]
fn timing_fd_pop_iy() {
    let mut c = create_z80(&[0xFD, 0xE1]);
    c.sp = 0x100;
    assert_eq!(c.step(), 14);
}
#[test]
fn timing_fd_ex_sp_iy() {
    let mut c = create_z80(&[0xFD, 0xE3]);
    c.sp = 0x100;
    assert_eq!(c.step(), 23);
}
#[test]
fn timing_fd_push_iy() {
    let mut c = create_z80(&[0xFD, 0xE5]);
    c.sp = 0x200;
    assert_eq!(c.step(), 15);
}
#[test]
...
#[test]
fn timing_fdcb_rlc() {
    let mut c = create_z80(&[0xFD, 0xCB, 0x00, 0x06]);
    c.iy = 0x100;
    assert_eq!(c.step(), 23);
}
#[test]
fn timing_fdcb_bit() {
    let mut c = create_z80(&[0xFD, 0xCB, 0x00, 0x46]);
    c.iy = 0x100;
    assert_eq!(c.step(), 20);
}
#[test]
fn timing_fdcb_res() {
    let mut c = create_z80(&[0xFD, 0xCB, 0x00, 0x86]);
    c.iy = 0x100;
    assert_eq!(c.step(), 23);
}
#[test]
fn timing_fdcb_set() {
    let mut c = create_z80(&[0xFD, 0xCB, 0x00, 0xC6]);
    c.iy = 0x100;
    assert_eq!(c.step(), 23);
}
