...
fn daa_bcd_99_plus_1() {
    ...
    let (mut c, mut context) = z80(&[0x80, 0x27]); // ADD A, B; DAA
    c.a = 0x99;
    c.b = 0x01;
    c.step(&mut context); // ADD
...
fn daa_bcd_45_plus_37() {
    // 45 + 37 = 82 in BCD
    let (mut c, mut context) = z80(&[0x80, 0x27]);
    c.a = 0x45;
    c.b = 0x37;
    c.step(&mut context);
    c.step(&mut context);
...
fn daa_bcd_25_minus_50() {
    ...
    let (mut c, mut context) = z80(&[0x90, 0x27]);
    c.a = 0x25;
    c.b = 0x50;
    c.step(&mut context);
    c.step(&mut context);
...
fn daa_full_state_space() {
    ...
    for a in 0..=255 {
        for f in 0..=255 {
            ...
            let (mut c, mut context) = z80(&[0x27]); // DAA
            c.a = a_in;
            c.f = f_in;
            c.step(&mut context);
            ...
            if c.a != expected_a {
                ...
                if errors < 10 {
                    println!(
                        "DAA mismatch A: input A={:02x} F={:08b} | Exp={:02x} Got={:02x}",
                        a_in, f_in, expected_a, c.a
                    );
                }
                errors += 1;
            }
            if c.get_flag(flags::CARRY) != expected_c {
                if errors < 10 {
                    println!(
                        "DAA mismatch C: input A={:02x} F={:08b} | ExpC={} GotC={}",
                        a_in,
                        f_in,
                        expected_c,
    ...
    assert_eq!(
        errors, 0,
        "DAA exhaustive test failed with {} errors",
        errors
    );
}