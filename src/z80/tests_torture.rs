...
fn torture_im2_vector_fetch() {
    let (mut c, mut bus) = create_z80(&[0xED, 0x5E]); // IM 2
    c.i = 0x10;
    c.iff1 = true;
    c.step(&mut bus); // IM 2
    ...
    bus.memory.write_byte(0x10FF as u32, 0x30);
    bus.memory.write_byte(0x1100 as u32, 0x20);

    c.trigger_interrupt(&mut bus, 0xFF);
...
fn torture_ei_latency_shadow() {
    let (mut c, mut bus) = create_z80(&[0xFB, 0x3C, 0x3C]); // EI; INC A; INC A
    c.iff1 = false;
    c.step(&mut bus); // EI
    assert!(c.iff1);
    ...
    let cycles = c.trigger_interrupt(&mut bus, 0xFF);
    assert_eq!(cycles, 0);
    assert_eq!(c.pc, 0x0001); // Still at first INC A

    // After one instruction, shadow is gone
    c.step(&mut bus); // INC A
    assert!(!c.pending_ei);

    // Now interrupt should fire
    bus.memory.write_byte(0x0038 as u32, 0x00); // Handler at 0x0038 is NOP
    let cycles = c.trigger_interrupt(&mut bus, 0x00);
    assert!(cycles > 0);
...
fn torture_r_reg_bit7_preservation() {
    let (mut c, mut bus) = create_z80(&[0x00, 0x00]); // NOP; NOP
    c.r = 0x80; // Bit 7 set
    c.step(&mut bus); // Fetch NOP (increments R)
...
fn torture_bit_hl_memptr_leakage() {
    let (mut c, mut bus) = create_z80(&[0xCB, 0x46]); // BIT 0, (HL)
    c.memptr = 0x2800; // Bit 5 & 3 of high byte are 1
    c.set_hl(0x8000);
    bus.memory.write_byte(0x8000 as u32, 0x00);
    c.step(&mut bus);
...
fn torture_bit_ix_ea_leakage() {
    ...
    let (mut c, mut bus) = create_z80(&[0xDD, 0xCB, 0x28, 0x46]);
    c.ix = 0x2800; // EA = 0x2800 + 0x28 = 0x2828. High byte is 0x28.
    bus.memory.write_byte(0x2828 as u32, 0x00);
    c.step(&mut bus);
    assert!(c.get_flag(flags::X_FLAG));
    assert!(c.get_flag(flags::Y_FLAG));
}
...
fn torture_ldi_flags() {
    let (mut c, mut bus) = create_z80(&[0xED, 0xA0]); // LDI
    c.set_hl(0x1000);
    c.set_de(0x2000);
    c.set_bc(0x0002);
...
fn torture_flag_leakage_scf() {
    let (mut c, mut bus) = create_z80(&[0x37, 0x37]); // Two SCF instructions
    ...
    c.a = 0x00;
    c.step(&mut bus); // Execute second SCF with A=0
    assert!(!c.get_flag(flags::X_FLAG));
    assert!(!c.get_flag(flags::Y_FLAG));
}
...
fn torture_flag_leakage_rotates() {
    let (mut c, mut bus) = create_z80(&[0x07, 0x0F, 0x17, 0x1F]); // RLCA, RRCA, RLA, RRA
    c.a = 0x14; // bit 3 is 0, bit 5 is 0
    c.step(&mut bus); // RLCA -> A becomes 0x28 (bit 3=1, bit 5=1)
    assert!(c.get_flag(flags::X_FLAG));
    assert!(c.get_flag(flags::Y_FLAG));
}
...
fn torture_add_hl_memptr() {
    // ADD HL, BC; BIT 0, (HL)
    let (mut c, mut bus) = create_z80(&[0x09, 0xCB, 0x46]);
    c.set_hl(0x2800); // MEMPTR should become 0x2801
    c.set_bc(0x0001);
    bus.memory.write_byte(0x2801 as u32, 0x00);
    c.step(&mut bus); // ADD HL, BC
    assert_eq!(c.memptr, 0x2801);
    c.step(&mut bus); // BIT 0, (HL) -> leaks High byte of WZ (0x28)
