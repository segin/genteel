...
fn generate_ldir_length(rng: &mut Rng) -> u16 {
...
fn fill_ldir_memory_window<M: MemoryInterface, I: IoInterface>(
    cpu: &mut Z80<M, I>,
    ref_mem: &mut [u8],
    src: u16,
    dst: u16,
    ...
    for k in 0..256 {
        ...
        ref_mem[s_addr] = val;
        cpu.memory.write_byte(d_addr as u32, val ^ 0xFF);
...
fn execute_ldir_until_completion<M: MemoryInterface, I: IoInterface>(cpu: &mut Z80<M, I>, bc: u16) {
    let mut steps = 0;
    loop {
...
fn validate_ldir_state<M: MemoryInterface, I: IoInterface>(
    cpu: &mut Z80<M, I>,
    ref_mem: &[u8],
    exp_hl: u16,
    exp_de: u16,
...
fn run_ldir_test_case(i: usize, rng: &mut Rng) {
    ...
    let bc = generate_ldir_length(rng);
    ...
    fill_ldir_memory_window(&mut cpu, &mut ref_mem, src, dst, bc, rng);

    let (exp_hl, exp_de, exp_bc) = reference_ldir(&mut ref_mem, src, dst, bc);

    execute_ldir_until_completion(&mut cpu, bc);

    validate_ldir_state(&mut cpu, &ref_mem, exp_hl, exp_de, exp_bc, i, bc, rng);
}
...
fn test_ldir_exhaustive() {
    ...
    for i in 0..5000 {
        run_ldir_test_case(i, &mut rng);
    }
}

fn run_lddr_test_case(i: usize, rng: &mut Rng) {
    let src = rng.next_u16();
    let dst = rng.next_u16();
...
fn test_lddr_exhaustive() {
    ...
    for i in 0..2000 {
        run_lddr_test_case(i, &mut rng);
    }
}

fn run_cpir_test_case(i: usize, rng: &mut Rng) {
    let hl = rng.next_u16();
