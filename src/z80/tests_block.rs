...
fn setup_ldir_memory<M: MemoryInterface, I: crate::memory::IoInterface>(
    cpu: &mut Z80<M, I>,
    rng: &mut Rng,
    src: u16,
    dst: u16,
...
fn run_ldir_emulator_loop<M: MemoryInterface, I: crate::memory::IoInterface>(
    cpu: &mut Z80<M, I>,
    bc: u16,
) {
    let mut steps = 0;
...
fn run_ldir_test_case(i: usize, rng: &mut Rng) {
    ...
    let mut ref_mem = setup_ldir_memory(&mut cpu, rng, src, dst, bc);

    let (exp_hl, exp_de, exp_bc) = reference_ldir(&mut ref_mem, src, dst, bc);

    run_ldir_emulator_loop(&mut cpu, bc);

    validate_ldir_result(&mut cpu, rng, &ref_mem, i, bc, exp_hl, exp_de, exp_bc);
}
