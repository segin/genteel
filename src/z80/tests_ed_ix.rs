...
fn test_retn() {
    ...
    bus.memory.write_byte(0x1FFF as u32, 0x10);
    c.iff2 = true;
    c.step(&mut bus);
...
fn test_reti() {
    ...
    c.sp = 0x1FFE;
    bus.memory.write_byte(0x1FFE as u32, 0x00);
    bus.memory.write_byte(0x1FFF as u32, 0x20);
    c.step(&mut bus);
...
#[test]
fn test_adc_hl_de() {
    let (mut c, mut bus) = create_z80(&[0xED, 0x5A]);
    c.set_hl(0x8000);
    c.set_de(0x8000);
    c.f = 0;
    c.step(&mut bus);
...
#[test]
fn test_sbc_hl_de() {
    let (mut c, mut bus) = create_z80(&[0xED, 0x52]);
    c.set_hl(0x0100);
    c.set_de(0x0200);
    c.f = 0;
    c.step(&mut bus);
...
#[test]
fn test_ed_ld_nn_bc() {
    let (mut c, mut bus) = create_z80(&[0xED, 0x43, 0x00, 0x50]);
    c.set_bc(0x1234);
    c.step(&mut bus);
    assert_eq!(bus.memory.read_byte(0x5000 as u32), 0x34);
    assert_eq!(bus.memory.read_byte(0x5001 as u32), 0x12);
    assert_eq!(c.pc, 4);
}
#[test]
fn test_ed_ld_bc_nn() {
    let (mut c, mut bus) = create_z80(&[0xED, 0x4B, 0x00, 0x50]);
    bus.memory.write_byte(0x5000 as u32, 0xCD);
    bus.memory.write_byte(0x5001 as u32, 0xAB);
    c.step(&mut bus);
...
#[test]
fn test_ed_ld_nn_de() {
    let (mut c, mut bus) = create_z80(&[0xED, 0x53, 0x00, 0x60]);
    c.set_de(0x5678);
    c.step(&mut bus);
    assert_eq!(bus.memory.read_byte(0x6000 as u32), 0x78);
    assert_eq!(bus.memory.read_byte(0x6001 as u32), 0x56);
    assert_eq!(c.pc, 4);
}
#[test]
fn test_ed_ld_de_nn() {
    let (mut c, mut bus) = create_z80(&[0xED, 0x5B, 0x00, 0x60]);
    bus.memory.write_byte(0x6000 as u32, 0xEF);
    bus.memory.write_byte(0x6001 as u32, 0xBE);
    c.step(&mut bus);
...
#[test]
fn test_ed_ld_nn_sp() {
    let (mut c, mut bus) = create_z80(&[0xED, 0x73, 0x00, 0x70]);
    c.sp = 0x9ABC;
    c.step(&mut bus);
    assert_eq!(bus.memory.read_byte(0x7000 as u32), 0xBC);
    assert_eq!(bus.memory.read_byte(0x7001 as u32), 0x9A);
    assert_eq!(c.pc, 4);
}
#[test]
fn test_ed_ld_sp_nn() {
    let (mut c, mut bus) = create_z80(&[0xED, 0x7B, 0x00, 0x70]);
    bus.memory.write_byte(0x7000 as u32, 0xDE);
    bus.memory.write_byte(0x7001 as u32, 0xF0);
    c.step(&mut bus);
...
fn test_ldd() {
    ...
    c.set_de(0x2000);
    c.set_bc(0x0010);
    bus.memory.write_byte(0x1000 as u32, 0x55);
    c.step(&mut bus);
    assert_eq!(bus.memory.read_byte(0x2000 as u32), 0x55);
...
#[test]
fn test_ld_ix_nn() {
    let (mut c, mut bus) = create_z80(&[0xDD, 0x21, 0x34, 0x12]);
    c.step(&mut bus);
...
fn test_ld_ix_nn_ffff() {
    let (mut c, mut bus) = create_z80(&[0xDD, 0x21, 0xFF, 0xFF]);
    c.step(&mut bus);
...
#[test]
fn test_ld_nn_ix() {
    let (mut c, mut bus) = create_z80(&[0xDD, 0x22, 0x00, 0x50]);
    c.ix = 0xABCD;
    c.step(&mut bus);
    assert_eq!(bus.memory.read_byte(0x5000 as u32), 0xCD);
    assert_eq!(bus.memory.read_byte(0x5001 as u32), 0xAB);
    assert_eq!(c.pc, 4);
}
#[test]
fn test_ld_ix_nn_ind() {
    let (mut c, mut bus) = create_z80(&[0xDD, 0x2A, 0x00, 0x50]);
    bus.memory.write_byte(0x5000 as u32, 0x78);
    bus.memory.write_byte(0x5001 as u32, 0x56);
    c.step(&mut bus);
...
#[test]
fn test_ld_ix_d_n() {
    let (mut c, mut bus) = create_z80(&[0xDD, 0x36, 0x05, 0x42]);
    c.ix = 0x1000;
    c.step(&mut bus);
...
fn test_ld_ix_d_n_neg() {
    let (mut c, mut bus) = create_z80(&[0xDD, 0x36, 0xFB, 0x99]);
    c.ix = 0x1000;
    c.step(&mut bus);
    assert_eq!(bus.memory.read_byte(0x0FFB as u32), 0x99);
    assert_eq!(c.pc, 4);
}
#[test]
fn test_ld_b_ix_d() {
    let (mut c, mut bus) = create_z80(&[0xDD, 0x46, 0x10]);
    c.ix = 0x1000;
    bus.memory.write_byte(0x1010 as u32, 0xAB);
    c.step(&mut bus);
    assert_eq!(c.b, 0xAB);
    assert_eq!(c.pc, 3);
}
#[test]
fn test_ld_c_ix_d() {
    let (mut c, mut bus) = create_z80(&[0xDD, 0x4E, 0x20]);
    c.ix = 0x1000;
    bus.memory.write_byte(0x1020 as u32, 0xCD);
    c.step(&mut bus);
    assert_eq!(c.c, 0xCD);
    assert_eq!(c.pc, 3);
}
#[test]
fn test_ld_ix_d_b() {
    let (mut c, mut bus) = create_z80(&[0xDD, 0x70, 0x05]);
    c.ix = 0x2000;
    c.b = 0x11;
    c.step(&mut bus);
    assert_eq!(bus.memory.read_byte(0x2005 as u32), 0x11);
    assert_eq!(c.pc, 3);
}
#[test]
fn test_inc_ix_d() {
    let (mut c, mut bus) = create_z80(&[0xDD, 0x34, 0x10]);
    c.ix = 0x1000;
    bus.memory.write_byte(0x1010 as u32, 0x41);
    c.step(&mut bus);
    assert_eq!(bus.memory.read_byte(0x1010 as u32), 0x42);
    assert_eq!(c.pc, 3);
}
#[test]
fn test_dec_ix_d() {
    let (mut c, mut bus) = create_z80(&[0xDD, 0x35, 0x10]);
    c.ix = 0x1000;
    bus.memory.write_byte(0x1010 as u32, 0x42);
    c.step(&mut bus);
    assert_eq!(bus.memory.read_byte(0x1010 as u32), 0x41);
    assert_eq!(c.pc, 3);
}
...
#[test]
fn test_ld_iy_nn() {
    let (mut c, mut bus) = create_z80(&[0xFD, 0x21, 0xCD, 0xAB]);
    c.step(&mut bus);
...
fn test_push_iy() {
    ...
    c.iy = 0x5678;
    c.step(&mut bus);
    assert_eq!(bus.memory.read_byte(0x1FFE as u32), 0x78);
    assert_eq!(bus.memory.read_byte(0x1FFF as u32), 0x56);
    assert_eq!(c.pc, 2);
}
#[test]
fn test_pop_iy() {
    ...
    c.sp = 0x1FFE;
    bus.memory.write_byte(0x1FFE as u32, 0xEF);
    bus.memory.write_byte(0x1FFF as u32, 0xBE);
    c.step(&mut bus);
...
#[test]
fn test_ld_iy_d_n() {
    let (mut c, mut bus) = create_z80(&[0xFD, 0x36, 0x05, 0x77]);
    c.iy = 0x2000;
    c.step(&mut bus);
    assert_eq!(bus.memory.read_byte(0x2005 as u32), 0x77);
    assert_eq!(c.pc, 4);
}
#[test]
fn test_ld_a_iy_d() {
    let (mut c, mut bus) = create_z80(&[0xFD, 0x7E, 0x10]);
    c.iy = 0x2000;
    bus.memory.write_byte(0x2010 as u32, 0xEE);
    c.step(&mut bus);
    assert_eq!(c.a, 0xEE);
    assert_eq!(c.pc, 3);
}
#[test]
fn test_ld_iy_d_a() {
    let (mut c, mut bus) = create_z80(&[0xFD, 0x77, 0x05]);
    c.iy = 0x3000;
    c.a = 0x22;
    c.step(&mut bus);
    assert_eq!(bus.memory.read_byte(0x3005 as u32), 0x22);
    assert_eq!(c.pc, 3);
}
...
#[test]
fn test_dd_76_halt() {
    let (mut c, mut bus) = create_z80(&[0xDD, 0x76]);
...
#[test]
fn test_fd_76_halt() {
    let (mut c, mut bus) = create_z80(&[0xFD, 0x76]);
    let t = c.step(&mut bus);
    assert!(c.halted, "CPU should be halted");
    assert_eq!(c.pc, 2, "PC should be incremented by 2");
    assert_eq!(t, 8, "T-states should be 8");
