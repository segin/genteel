...
#[test]
fn test_adc_hl_de() {
    let mut c = create_z80(&[0xED, 0x5A]);
    c.set_hl(0x8000);
    c.set_de(0x8000);
    c.f = 0;
    c.step();
...
#[test]
fn test_sbc_hl_de() {
    let mut c = create_z80(&[0xED, 0x52]);
    c.set_hl(0x0100);
    c.set_de(0x0200);
    c.f = 0;
    c.step();
...
#[test]
fn test_ed_ld_nn_bc() {
    let mut c = create_z80(&[0xED, 0x43, 0x00, 0x50]);
    c.set_bc(0x1234);
    c.step();
...
#[test]
fn test_ed_ld_bc_nn() {
    let mut c = create_z80(&[0xED, 0x4B, 0x00, 0x50]);
...
#[test]
fn test_ed_ld_nn_de() {
    let mut c = create_z80(&[0xED, 0x53, 0x00, 0x60]);
    c.set_de(0x5678);
    c.step();
...
#[test]
fn test_ed_ld_de_nn() {
    let mut c = create_z80(&[0xED, 0x5B, 0x00, 0x60]);
...
#[test]
fn test_ed_ld_nn_sp() {
    let mut c = create_z80(&[0xED, 0x73, 0x00, 0x70]);
    c.sp = 0x9ABC;
    c.step();
...
fn test_ld_ix_nn_ffff() {
    let mut c = create_z80(&[0xDD, 0x21, 0xFF, 0xFF]);
    c.step();
...
#[test]
fn test_ld_nn_ix() {
    let mut c = create_z80(&[0xDD, 0x22, 0x00, 0x50]);
    c.ix = 0xABCD;
    c.step();
...
fn test_ld_ix_nn_ind() {
    let mut c = create_z80(&[0xDD, 0x2A, 0x00, 0x50]);
...
#[test]
fn test_ld_ix_d_n() {
    let mut c = create_z80(&[0xDD, 0x36, 0x05, 0x42]);
    c.ix = 0x1000;
    c.step();
...
fn test_ld_ix_d_n_neg() {
    let mut c = create_z80(&[0xDD, 0x36, 0xFB, 0x99]);
    c.ix = 0x1000;
    c.step();
...
#[test]
fn test_ld_b_ix_d() {
    let mut c = create_z80(&[0xDD, 0x46, 0x10]);
    c.ix = 0x1000;
...
#[test]
fn test_ld_c_ix_d() {
    let mut c = create_z80(&[0xDD, 0x4E, 0x20]);
    c.ix = 0x1000;
...
#[test]
fn test_ld_ix_d_b() {
    let mut c = create_z80(&[0xDD, 0x70, 0x05]);
    c.ix = 0x2000;
    c.b = 0x11;
    c.step();
...
#[test]
fn test_inc_ix_d() {
    let mut c = create_z80(&[0xDD, 0x34, 0x10]);
    c.ix = 0x1000;
...
#[test]
fn test_dec_ix_d() {
    let mut c = create_z80(&[0xDD, 0x35, 0x10]);
    c.ix = 0x1000;
...
#[test]
fn test_ld_iy_nn() {
    let mut c = create_z80(&[0xFD, 0x21, 0xCD, 0xAB]);
    c.step();
...
#[test]
fn test_ld_iy_d_n() {
    let mut c = create_z80(&[0xFD, 0x36, 0x05, 0x77]);
    c.iy = 0x2000;
    c.step();
...
#[test]
fn test_ld_a_iy_d() {
    let mut c = create_z80(&[0xFD, 0x7E, 0x10]);
    c.iy = 0x2000;
...
#[test]
fn test_ld_iy_d_a() {
    let mut c = create_z80(&[0xFD, 0x77, 0x05]);
    c.iy = 0x3000;
    c.a = 0x22;
    c.step();
...
#[test]
fn test_fd_76_halt() {
    let mut c = create_z80(&[0xFD, 0x76]);
    let t = c.step();
    assert!(c.halted, "CPU should be halted");
    assert_eq!(c.pc, 2, "PC should be incremented by 2");
    assert_eq!(t, 8, "T-states should be 8");
