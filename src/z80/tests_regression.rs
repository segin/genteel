...
fn regression_daa_after_sub() {
    let (mut c, mut context) = z80(&[0x90, 0x27]); // SUB B; DAA
    c.a = 0x50;
    c.b = 0x25;
    c.step(&mut context); // SUB
    c.step(&mut context); // DAA
    assert_eq!(c.a, 0x25);
}
...
fn regression_jr_negative() {
    let (mut c, mut context) = z80(&[0x00, 0x00, 0x18, 0xFC]); // JR -4
    c.pc = 2;
    c.step(&mut context);
...
fn regression_ld_hl_h() {
    let (mut c, mut context) = z80(&[0x74]); // LD (HL), H
    c.set_hl(0x1234);
    c.step(&mut context);
    assert_eq!(context.read_byte(0x1234 as u32), 0x12); // H value, not modified
}

// Bug: PUSH/POP AF not preserving all flag bits
#[test]
fn regression_push_pop_af_all_bits() {
    let (mut c, mut context) = z80(&[0xF5, 0xF1]); // PUSH AF; POP AF
    c.sp = 0x8000;
    c.a = 0xFF;
    c.f = 0xFF; // All flag bits set
    c.step(&mut context); // PUSH
    c.a = 0x00;
    c.f = 0x00;
    c.step(&mut context); // POP
    assert_eq!(c.a, 0xFF);
    assert_eq!(c.f, 0xFF);
}
...
fn regression_ex_sp_hl() {
    ...
    c.sp = 0x1000;
    c.set_hl(0x1234);
    context.write_byte(0x1000 as u32, 0xCD);
    context.write_byte(0x1001 as u32, 0xAB);
    c.step(&mut context);
    assert_eq!(c.hl(), 0xABCD);
    assert_eq!(context.read_byte(0x1000 as u32), 0x34);
    assert_eq!(context.read_byte(0x1001 as u32), 0x12);
}

// Bug: INC/DEC not affecting V flag correctly
#[test]
fn regression_inc_overflow() {
    let (mut c, mut context) = z80(&[0x3C]); // INC A
    c.a = 0x7F;
    c.step(&mut context);
    assert_eq!(c.a, 0x80);
...
fn regression_dec_overflow() {
    let (mut c, mut context) = z80(&[0x3D]); // DEC A
    c.a = 0x80;
    c.step(&mut context);
    assert_eq!(c.a, 0x7F);
    assert!(c.get_flag(flags::PARITY)); // Overflow
...
fn regression_scf_clears_h() {
    let (mut c, mut context) = z80(&[0x37]); // SCF
...
fn regression_ccf_copies_c_to_h() {
    let (mut c, mut context) = z80(&[0x3F]); // CCF
...
fn regression_ldir_bc_zero() {
    ...
    assert_eq!(context.read_byte(0x2000 as u32), 0xAA);
}
...
fn regression_bit_h_flag() {
    let (mut c, mut context) = z80(&[0xCB, 0x47]); // BIT 0, A
    c.a = 0x00;
    c.step(&mut context);
    assert!(c.get_flag(flags::HALF_CARRY));
}
...
fn regression_rlc_parity() {
    let (mut c, mut context) = z80(&[0xCB, 0x07]); // RLC A
    c.a = 0x00;
    c.step(&mut context);
    assert!(c.get_flag(flags::ZERO));
...
fn regression_sp_wrap_push() {
    let (mut c, mut context) = z80(&[0xC5]); // PUSH BC
    c.sp = 0x0001;
    c.set_bc(0x1234);
    c.step(&mut context);
    assert_eq!(c.sp, 0xFFFF);
    assert_eq!(context.read_byte(0xFFFF as u32), 0x34);
    assert_eq!(context.read_byte(0x0000 as u32), 0x12);
}

#[test]
fn regression_sp_wrap_pop() {
    let (mut c, mut context) = z80(&[0xC1]); // POP BC at addr 0
    c.sp = 0xFFFE; // Use 0xFFFE so we don't overwrite the instruction
    context.write_byte(0xFFFE as u32, 0xCD);
    context.write_byte(0xFFFF as u32, 0xAB);
    c.step(&mut context);
...
fn regression_pc_wrap() {
    let (mut c, mut context) = z80(&[0x00]); // NOP at 0xFFFF
    c.pc = 0xFFFF;
    context.write_byte(0xFFFF as u32, 0x00);
    c.step(&mut context);
...
fn regression_ei_di_sequence() {
    ...
    let (mut c, mut context) = z80(&[0xFB, 0xF3]); // EI; DI
    c.iff1 = false;
    c.iff2 = false;
    c.step(&mut context); // EI
    c.step(&mut context); // DI
    assert!(!c.iff1);
    assert!(!c.iff2);
}
