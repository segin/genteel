...
fn rrd_basic() {
    ...
    c.a = 0x12;
    c.set_hl(0x1000);
    context.write_byte(0x1000 as u32, 0x34);
    c.step(&mut context);
    assert_eq!(c.a, 0x14);
    assert_eq!(context.read_byte(0x1000 as u32), 0x23);
}
...
fn rrd_zeros() {
    ...
    c.step(&mut context);
    assert_eq!(c.a, 0x00);
    assert_eq!(context.read_byte(0x1000 as u32), 0x00);
}
...
fn rrd_all_ones() {
    ...
    c.a = 0xFF;
    c.set_hl(0x1000);
    context.write_byte(0x1000 as u32, 0xFF);
    c.step(&mut context);
    assert_eq!(c.a, 0xFF);
    assert_eq!(context.read_byte(0x1000 as u32), 0xFF);
}
...
fn rrd_a_high_nibble_preserved() {
    ...
    c.a = 0xF0;
    c.set_hl(0x1000);
    context.write_byte(0x1000 as u32, 0x12);
    c.step(&mut context);
...
fn rld_basic() {
    ...
    c.step(&mut context);
    assert_eq!(c.a, 0x13);
    assert_eq!(context.read_byte(0x1000 as u32), 0x42);
}
...
fn rrd_rld_roundtrip() {
    ...
    let (mut c, mut context) = z80(&[0xED, 0x67, 0xED, 0x6F]); // RRD, RLD
    c.a = 0x12;
    c.set_hl(0x1000);
    context.write_byte(0x1000 as u32, 0x34);
    ...
    // Store original
    let orig_a = c.a;
    let orig_mem = context.read_byte(0x1000 as u32);

    c.step(&mut context); // RRD
    c.step(&mut context); // RLD

    assert_eq!(c.a, orig_a);
    assert_eq!(context.read_byte(0x1000 as u32), orig_mem);
}

#[test]
fn rld_rrd_roundtrip() {
    // Applying RLD then RRD should give original values
    let (mut c, mut context) = z80(&[0xED, 0x6F, 0xED, 0x67]); // RLD, RRD
    c.a = 0x56;
    c.set_hl(0x1000);
    context.write_byte(0x1000 as u32, 0x78);
...
fn rrd_parity_odd() {
    ...
    c.a = 0x00;
    c.set_hl(0x1000);
    context.write_byte(0x1000 as u32, 0x10); // A becomes 0x00, (HL) becomes 0x01, A's low = 0
    c.step(&mut context);
