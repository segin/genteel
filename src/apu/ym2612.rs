...
pub struct Ym2612 {
    ...
    phase: [f32; 6],
    /// Pre-calculated phase increments for the 6 channels
    phase_inc: [f32; 6],
...
impl Ym2612 {
    ...
    pub fn write_data_bank(&mut self, bank: Bank, val: u8) {
        ...
        if bank == Bank::Bank0 && addr == 0x27 {
            ...
            // Check if this is a frequency register write and update phase increment
            // Bank 0: 0xA0-0xA2, 0xA4-0xA6 (Channels 0-2)
            // Bank 1: 0xA0-0xA2, 0xA4-0xA6 (Channels 3-5)
            if (0xA0..=0xA2).contains(&addr) || (0xA4..=0xA6).contains(&addr) {
                let offset = if addr >= 0xA4 { addr - 0xA4 } else { addr - 0xA0 };
                let channel = offset as usize + if bank == Bank::Bank1 { 3 } else { 0 };
                self.update_phase_inc(channel);
            }
        }
    }
