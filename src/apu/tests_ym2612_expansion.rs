...
fn test_ym2612_all_channels_enable() {
    ...
    for ch in 0..6 {
        let (bank, offset) = if ch < 3 { (Bank::Bank0, ch) } else { (Bank::Bank1, ch - 3) };
    ...
    assert!(l != 0 || r != 0, "Samples should be non-zero when channels are active");
}
...
fn test_ym2612_timer_ab_simultaneous() {
    ...
    // Timer A: period 144
    ym.write_addr(Bank::Bank0, 0x24); ym.write_data_bank(Bank::Bank0, 0xFF);
    ym.write_addr(Bank::Bank0, 0x25); ym.write_data_bank(Bank::Bank0, 0x03);
    
    // Timer B: period 2304
    ym.write_addr(Bank::Bank0, 0x26); ym.write_data_bank(Bank::Bank0, 0xFF);
