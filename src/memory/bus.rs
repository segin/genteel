...
impl Debuggable for Bus {
    ...
    fn write_state(&mut self, state: &Value) {
        if let Ok(bus_state) = BusJsonState::deserialize(state) {
...
mod tests {
    ...
    fn test_bus_state_serialization() {
        ...
        let partial_state = serde_json::json!({
            "z80_bus_request": false
        });
        bus2.write_state(&partial_state);
        assert_eq!(bus2.z80_bus_request, false);
        assert_eq!(bus2.z80_reset, false); // Unchanged
    }
}
