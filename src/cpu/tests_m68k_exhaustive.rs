...
fn ref_add(src: u32, dst: u32, size: Size) -> (u32, u16) {
    ...
    let mut flags = 0u16;
    if n { flags |= flags::NEGATIVE; }
    if z { flags |= flags::ZERO; }
    if v { flags |= flags::OVERFLOW; }
    if c { flags |= flags::CARRY; }
    if x { flags |= flags::EXTEND; }
    (res, flags)
}
...
fn exhaustive_m68k_add() {
    ...
    for (s_idx, &size) in sizes.iter().enumerate() {
        for i in 0..1000 {
            ...
            assert_eq!(cpu.d[0], exp_res, "ADD.{} iter {}: result mismatch (dst_in={:08X}, src={:08X})", size, i, a, b);
            assert_eq!(cpu.sr & 0x1F, exp_sr, "ADD.{} iter {}: flags mismatch (dst_in={:08X}, src={:08X})", size, i, a, b);
        }
    }
}
...
fn exhaustive_m68k_sub() {
    ...
    for (s_idx, &size) in sizes.iter().enumerate() {
        for i in 0..1000 {
            ...
            assert_eq!(cpu.d[0], exp_res, "SUB.{} iter {}: result mismatch (dst_in={:08X}, src={:08X})", size, i, a, b);
            assert_eq!(cpu.sr & 0x1F, exp_sr, "SUB.{} iter {}: flags mismatch (dst_in={:08X}, src={:08X})", size, i, a, b);
        }
    }
}
...
fn exhaustive_m68k_logic() {
    ...
    for (name, opcode, size) in test_configs {
        for i in 0..1000 {
            ...
            assert_eq!(actual_res, exp_res, "{} iter {}: result mismatch (D0={:08X}, D1={:08X})", name, i, a, b);
            assert_eq!(cpu.sr & 0x1F, exp_sr, "{} iter {}: flags mismatch (D0={:08X}, D1={:08X})", name, i, a, b);
        }
    }
}
...
fn torture_m68k_address_error() {
    ...
    assert_eq!(cpu.pc, 0x2000, "Should trap to Address Error handler on unaligned access");
    assert!(cpu.get_flag(flags::SUPERVISOR), "Should be in supervisor mode after trap");
}
