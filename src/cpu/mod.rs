...
impl Cpu {
    ...
    pub(crate) fn fetch_bit_num(&mut self, bit: BitSource, memory: &mut dyn MemoryInterface) -> u8 {
        match bit {
            BitSource::Immediate => {
    ...
    // === Stack Helpers ===
    pub(crate) fn push_long(&mut self, val: u32, memory: &mut dyn MemoryInterface) {
        let addr = self.a[7].wrapping_sub(4);
        self.a[7] = addr;
        self.write_long(addr, val, memory);
    }

    pub(crate) fn push_word(&mut self, val: u16, memory: &mut dyn MemoryInterface) {
        let addr = self.a[7].wrapping_sub(2);
        self.a[7] = addr;
        self.write_word(addr, val, memory);
    }

    pub(crate) fn pop_long(&mut self, memory: &mut dyn MemoryInterface) -> u32 {
        let addr = self.a[7];
    ...
    pub(crate) fn pop_word(&mut self, memory: &mut dyn MemoryInterface) -> u16 {
        let addr = self.a[7];
    ...
    pub(crate) fn read_word(&mut self, addr: u32, memory: &mut dyn MemoryInterface) -> u16 {
        if addr % 2 != 0 {
            self.process_exception(3, memory); // Address Error
            return 0;
        }
    ...
    pub(crate) fn read_long(&mut self, addr: u32, memory: &mut dyn MemoryInterface) -> u32 {
        if addr % 2 != 0 {
            self.process_exception(3, memory); // Address Error
            return 0;
        }
    ...
    pub(crate) fn write_word(&mut self, addr: u32, val: u16, memory: &mut dyn MemoryInterface) {
        if addr % 2 != 0 {
            self.process_exception(3, memory); // Address Error
            return;
        }
    ...
    pub(crate) fn write_long(&mut self, addr: u32, val: u32, memory: &mut dyn MemoryInterface) {
        if addr % 2 != 0 {
            self.process_exception(3, memory); // Address Error
            return;
        }
    ...
    pub(crate) fn cpu_read_memory(&mut self, addr: u32, size: Size, memory: &mut dyn MemoryInterface) -> u32 {
        match size {
    ...
    pub(crate) fn cpu_write_memory(&mut self, addr: u32, size: Size, val: u32, memory: &mut dyn MemoryInterface) {
        match size {
    ...
    pub(crate) fn cpu_read_ea(&mut self, ea: EffectiveAddress, size: Size, memory: &mut dyn MemoryInterface) -> u32 {
    ...
    pub(crate) fn cpu_write_ea(&mut self, ea: EffectiveAddress, size: Size, val: u32, memory: &mut dyn MemoryInterface) {
        if let EffectiveAddress::Memory(addr) = ea {
            if size != Size::Byte && addr % 2 != 0 {
                self.process_exception(3, memory);
                return;
    ...
    pub(crate) fn process_exception(&mut self, vector: u32, memory: &mut dyn MemoryInterface) -> u32 {
