...
use addressing::EffectiveAddress;

pub const CACHE_SIZE: usize = 0x10000;
pub const CACHE_MASK: u32 = (CACHE_SIZE as u32) - 1;
pub const CACHE_ROM_LIMIT: u32 = 0x400000;
...
impl Cpu {
    ...
    pub fn cpu_read_memory<M: MemoryInterface>(&mut self, address: u32, size: Size, memory: &mut M) -> u32 {
        memory.read_size(address, size)
    }
    ...
    pub fn cpu_write_memory<M: MemoryInterface>(&mut self, address: u32, size: Size, value: u32, memory: &mut M) {
        memory.write_size(address, value, size);
    }
    ...
    pub fn check_interrupts<M: MemoryInterface>(&mut self, memory: &mut M) -> u32 {
        ...
        if self.pending_interrupt > mask as u8 {
            ...
            // This pushes PC and old SR, and sets S-bit and clears T-bit.
            let cycles = self.process_exception(vector, memory);

            // Interrupt specific: update mask to current level
            self.sr = (self.sr & !0x0700) | ((level as u16) << 8);

            // Acknowledge interrupt (clear pending bit for this level)
            self.acknowledge_interrupt(level);

            // Exception processing takes ~34 cycles, interrupts take ~44.
            // process_exception returns 34.
            return 44;
        }
        0
    }
