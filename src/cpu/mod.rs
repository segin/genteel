...
impl Cpu {
    pub fn new<M: MemoryInterface>(memory: &mut M) -> Self {
        let mut cpu = Self {
            ...
            decode_cache: vec![DecodeCacheEntry::default(); 2_097_152].into_boxed_slice(),
        };
    ...
    fn invalidate_cache_line(&mut self, addr: u32) {
        // Only invalidate if address is within cacheable ROM space (0-4MB)
        if addr < 0x400000 {
            let index = ((addr >> 1) & 0x1FFFFF) as usize;
            // Safety: index is masked to cache size
    ...
    pub fn step_instruction<M: MemoryInterface>(&mut self, memory: &mut M) -> u32 {
        ...
        if pc < 0x400000 {
            ...
            // Index: (PC / 2) & 0x1FFFFF. Maps full 4MB range.
            // Since we check entry.pc == pc, aliasing is handled safely.
            let cache_index = ((pc >> 1) & 0x1FFFFF) as usize;

            // Safety: cache size is 2_097_152, index is masked to 0x1FFFFF.
    ...
    pub(crate) fn fetch_bit_num<M: MemoryInterface>(
        &mut self,
        bit: BitSource,
        memory: &mut M,
    ) -> u8 {
    ...
    pub(crate) fn cpu_read_memory<M: MemoryInterface>(
        &mut self,
        addr: u32,
        size: Size,
        memory: &mut M,
    ...
    pub(crate) fn cpu_write_memory<M: MemoryInterface>(
        &mut self,
        addr: u32,
        size: Size,
        val: u32,
    ...
    pub(crate) fn cpu_write_ea<M: MemoryInterface>(
        &mut self,
        ea: EffectiveAddress,
        size: Size,
        val: u32,
    ...
    pub(crate) fn process_exception<M: MemoryInterface>(
        &mut self,
        vector: u32,
        memory: &mut M,
    ) -> u32 {
