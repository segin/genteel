...
fn test_add_b_d0_d1() {
    ...
    write_op(&mut cpu, &[0xD200]); // ADD.B D0, D1
...
fn test_add_w_d0_d1() {
    let mut cpu = create_cpu();
    write_op(&mut cpu, &[0xD240]); // ADD.W D0, D1
...
fn test_add_l_d0_d1() {
    let mut cpu = create_cpu();
    write_op(&mut cpu, &[0xD280]); // ADD.L D0, D1
...
fn test_addq_to_address_reg() {
    let mut cpu = create_cpu();
    write_op(&mut cpu, &[0x5248]); // ADDQ.W #1, A0
    cpu.a[0] = 0x1000;
    cpu.step_instruction();
...
fn test_adda_w() {
    let mut cpu = create_cpu();
    write_op(&mut cpu, &[0xD0C0]); // ADDA.W D0, A0
...
fn test_adda_l() {
    let mut cpu = create_cpu();
    write_op(&mut cpu, &[0xD1C0]); // ADDA.L D0, A0
...
fn test_addx_register() {
    let mut cpu = create_cpu();
    write_op(&mut cpu, &[0xD101]); // ADDX.B D1, D0
...
fn test_sub_b_d0_d1() {
    let mut cpu = create_cpu();
    write_op(&mut cpu, &[0x9200]); // SUB.B D0, D1
...
fn test_subx_register() {
    let mut cpu = create_cpu();
    write_op(&mut cpu, &[0x9101]); // SUBX.B D1, D0
...
fn test_neg_b() {
    let mut cpu = create_cpu();
    write_op(&mut cpu, &[0x4400]); // NEG.B D0
    cpu.d[0] = 0x01;
    cpu.step_instruction();
...
fn test_negx_with_extend() {
    let mut cpu = create_cpu();
    write_op(&mut cpu, &[0x4000]); // NEGX.B D0
    cpu.d[0] = 0x00;
    cpu.set_flag(flags::EXTEND, true);
    cpu.step_instruction();
...
fn test_cmp_equal() {
    let mut cpu = create_cpu();
    write_op(&mut cpu, &[0xB200]); // CMP.B D0, D1
...
fn test_and_b() {
    let mut cpu = create_cpu();
    write_op(&mut cpu, &[0xC200]); // AND.B D0, D1
...
fn test_and_l() {
    let mut cpu = create_cpu();
    write_op(&mut cpu, &[0xC280]); // AND.L D0, D1
...
fn test_or_b() {
    let mut cpu = create_cpu();
    write_op(&mut cpu, &[0x8200]); // OR.B D0, D1
    cpu.d[0] = 0xF0; cpu.d[1] = 0x0F;
    cpu.step_instruction();
    assert_eq!(cpu.d[1] & 0xFF, 0xFF);
}
...
fn test_or_l() {
    let mut cpu = create_cpu();
    write_op(&mut cpu, &[0x8280]); // OR.L D0, D1
...
fn test_eor_b() {
    let mut cpu = create_cpu();
    write_op(&mut cpu, &[0xB300]); // EOR.B D1, D0
...
fn test_eor_l() {
    let mut cpu = create_cpu();
    write_op(&mut cpu, &[0xB380]); // EOR.L D1, D0
...
fn test_not_b() {
    let mut cpu = create_cpu();
    write_op(&mut cpu, &[0x4600]); // NOT.B D0
    cpu.d[0] = 0x55;
    cpu.step_instruction();
...
fn test_not_l() {
    let mut cpu = create_cpu();
    write_op(&mut cpu, &[0x4680]); // NOT.L D0
    cpu.d[0] = 0x00000000;
    cpu.step_instruction();
    assert_eq!(cpu.d[0], 0xFFFFFFFF);
}
...
fn test_mulu_basic() {
    let mut cpu = create_cpu();
    write_op(&mut cpu, &[0xC0C1]); // MULU.W D1, D0
...
fn test_muls_pos_pos() {
    let mut cpu = create_cpu();
    write_op(&mut cpu, &[0xC1C1]); // MULS.W D1, D0
    cpu.d[0] = 100; cpu.d[1] = 200;
    cpu.step_instruction();
    assert_eq!(cpu.d[0], 20000);
}
...
fn test_divu_basic() {
    let mut cpu = create_cpu();
    write_op(&mut cpu, &[0x80C1]); // DIVU.W D1, D0
...
fn test_divs_neg_pos() {
    let mut cpu = create_cpu();
    write_op(&mut cpu, &[0x81C1]); // DIVS.W D1, D0
...
fn test_divu_by_zero() {
    ...
    cpu.memory.write_long(0x14, 0x2000);
    cpu.step_instruction();
    // Should trap
}
