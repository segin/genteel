...
fn decode_bit_dynamic(opcode: u16) -> Option<Instruction> {
    if opcode & 0x0100 != 0 {
        ...
        if let Some(dst) = AddressingMode::from_mode_reg(mode, ea_reg) {
            ...
            return Some(Instruction::Bits(match op {
                0b00 => BitsInstruction::Btst { bit, dst },
                0b01 => BitsInstruction::Bchg { bit, dst },
                0b10 => BitsInstruction::Bclr { bit, dst },
                0b11 => BitsInstruction::Bset { bit, dst },
                _ => unreachable!(),
            }));
        }
    }
...
fn decode_static_bit(opcode: u16) -> Option<Instruction> {
    ...
    if op == 0b100 {
        ...
        if let Some(dst) = AddressingMode::from_mode_reg(mode, reg) {
            ...
            return Some(Instruction::Bits(match bit_op {
                0b00 => BitsInstruction::Btst { bit, dst },
                0b01 => BitsInstruction::Bchg { bit, dst },
                0b10 => BitsInstruction::Bclr { bit, dst },
                0b11 => BitsInstruction::Bset { bit, dst },
...
fn decode_ccr_sr_immediate(opcode: u16) -> Option<Instruction> {
    ...
    if mode == 7 && reg == 4 {
        ...
        return Some(Instruction::System(match (op, size_bits) {
            (0b000, 0b00) => SystemInstruction::OriToCcr,
            (0b000, 0b01) => SystemInstruction::OriToSr,
            (0b001, 0b00) => SystemInstruction::AndiToCcr,
            (0b001, 0b01) => SystemInstruction::AndiToSr,
            (0b101, 0b00) => SystemInstruction::EoriToCcr,
            (0b101, 0b01) => SystemInstruction::EoriToSr,
            _ => return None,
        }));
    }
    None
...
fn decode_group_4_misc(opcode: u16) -> Option<Instruction> {
    ...
    match opcode & 0xFFF8 {
        0x4E70 => {
            return Some(Instruction::System(match reg {
                0 => SystemInstruction::Reset,
                1 => SystemInstruction::Nop,
                2 => SystemInstruction::Stop,
                3 => SystemInstruction::Rte,
                5 => SystemInstruction::Rts,
                6 => SystemInstruction::TrapV,
                7 => SystemInstruction::Rtr,
                _ => return None,
            }))
        }
        ...
        0x4E60 => return Some(Instruction::System(SystemInstruction::MoveUsp { reg, to_usp: true })),
        0x4E68 => return Some(Instruction::System(SystemInstruction::MoveUsp { reg, to_usp: false })),
...
fn decode_group_4_control(opcode: u16) -> Option<Instruction> {
    ...
    if opcode & 0xF1C0 == 0x4180 {
        let dst_reg = ((opcode >> 9) & 0x07) as u8;
        if let Some(src) = AddressingMode::from_mode_reg(mode, reg) {
            return Some(Instruction::Arithmetic(ArithmeticInstruction::Chk { src, dst_reg }));
        }
    }
    None
}
...
fn decode_group_4_arithmetic(opcode: u16) -> Option<Instruction> {
    ...
    if let Some(size) = Size::from_bits(bits_7_6 as u8) {
        if let Some(dst) = AddressingMode::from_mode_reg(mode, reg) {
            match bits_11_8 {
                0x0 => return Some(Instruction::Arithmetic(ArithmeticInstruction::NegX { size, dst })),
                0x2 => return Some(Instruction::Data(DataInstruction::Clr { size, dst })),
                0x4 => return Some(Instruction::Arithmetic(ArithmeticInstruction::Neg { size, dst })),
                0x6 => return Some(Instruction::Bits(BitsInstruction::Not { size, dst })),
                0xA => return Some(Instruction::Arithmetic(ArithmeticInstruction::Tst { size, dst })),
                _ => {}
            };
        }
    }
...
fn make_shift_instruction(
    ...
    Instruction::Bits(match (op_type, direction) {
        (0b00, false) => BitsInstruction::Asr { size, dst, count },
        (0b00, true) => BitsInstruction::Asl { size, dst, count },
        (0b01, false) => BitsInstruction::Lsr { size, dst, count },
        (0b01, true) => BitsInstruction::Lsl { size, dst, count },
        (0b10, false) => BitsInstruction::Roxr { size, dst, count },
        (0b10, true) => BitsInstruction::Roxl { size, dst, count },
        (0b11, false) => BitsInstruction::Ror { size, dst, count },
        (0b11, true) => BitsInstruction::Rol { size, dst, count },
...
mod tests {
    ...
    fn test_decode_bra() {
        ...
        assert_eq!(instr, Instruction::System(SystemInstruction::Bra { displacement: 16 }));
    }
