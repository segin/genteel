...
fn test_asl_b_by_1() {
    ...
    write_op(&mut cpu, &[0xE300]); // ASL.B #1, D0
...
fn test_asl_w_by_8() {
    let mut cpu = create_cpu();
    write_op(&mut cpu, &[0xE140]); // ASL.W #8, D0 (encoded as 0, meaning 8)
    cpu.d[0] = 0x00FF;
    cpu.step_instruction();
...
fn test_asl_l_by_1() {
    let mut cpu = create_cpu();
    write_op(&mut cpu, &[0xE380]); // ASL.L #1, D0
    cpu.d[0] = 0x40000000;
    cpu.step_instruction();
...
fn test_asl_register_count() {
    let mut cpu = create_cpu();
    write_op(&mut cpu, &[0xE320]); // ASL.B D1, D0
    cpu.d[0] = 0x01;
    cpu.d[1] = 4;
    cpu.step_instruction();
...
fn test_asr_b_by_1() {
    let mut cpu = create_cpu();
    write_op(&mut cpu, &[0xE200]); // ASR.B #1, D0
...
fn test_asr_sign_preserve() {
    let mut cpu = create_cpu();
    write_op(&mut cpu, &[0xE600]); // ASR.B #3, D0
...
fn test_lsl_b_by_1() {
    let mut cpu = create_cpu();
    write_op(&mut cpu, &[0xE308]); // LSL.B #1, D0
    cpu.d[0] = 0x40;
    cpu.step_instruction();
    assert_eq!(cpu.d[0] & 0xFF, 0x80);
}
...
fn test_lsr_b_by_1() {
    let mut cpu = create_cpu();
    write_op(&mut cpu, &[0xE208]); // LSR.B #1, D0
    cpu.d[0] = 0x80;
    cpu.step_instruction();
...
fn test_rol_b_by_1() {
    let mut cpu = create_cpu();
    write_op(&mut cpu, &[0xE318]); // ROL.B #1, D0
    cpu.d[0] = 0x80;
    cpu.step_instruction();
...
fn test_rol_b_full_rotation() {
    let mut cpu = create_cpu();
    write_op(&mut cpu, &[0xE118]); // ROL.B #8, D0
    cpu.d[0] = 0xAB;
    cpu.step_instruction();
...
fn test_rol_w() {
    let mut cpu = create_cpu();
    write_op(&mut cpu, &[0xE358]); // ROL.W #1, D0
    cpu.d[0] = 0x8000;
    cpu.step_instruction();
...
fn test_ror_b_by_1() {
    let mut cpu = create_cpu();
    write_op(&mut cpu, &[0xE218]); // ROR.B #1, D0
    cpu.d[0] = 0x01;
    cpu.step_instruction();
    assert_eq!(cpu.d[0] & 0xFF, 0x80);
    assert!(cpu.get_flag(flags::CARRY));
...
fn test_ror_b_full_rotation() {
    let mut cpu = create_cpu();
    write_op(&mut cpu, &[0xE018]); // ROR.B #8, D0
    cpu.d[0] = 0xAB;
    cpu.step_instruction();
...
fn test_roxl_b_by_1() {
    let mut cpu = create_cpu();
    write_op(&mut cpu, &[0xE310]); // ROXL.B #1, D0
    cpu.d[0] = 0x80;
...
fn test_roxr_b_by_1() {
    let mut cpu = create_cpu();
    write_op(&mut cpu, &[0xE210]); // ROXR.B #1, D0
    cpu.d[0] = 0x01;
    cpu.set_flag(flags::EXTEND, false);
    cpu.step_instruction();
    assert_eq!(cpu.d[0] & 0xFF, 0x00);
...
fn test_asl_memory() {
    let mut cpu = create_cpu();
    write_op(&mut cpu, &[0xE1D0]); // ASL (A0)
    cpu.a[0] = 0x2000;
...
fn test_lsr_memory() {
    let mut cpu = create_cpu();
    write_op(&mut cpu, &[0xE2D0]); // LSR (A0)
    cpu.a[0] = 0x2000;
    cpu.memory.write_word(0x2000, 0x8000);
    cpu.step_instruction();
    assert_eq!(cpu.memory.read_word(0x2000), 0x4000);
}
...
fn test_shift_by_zero() {
    let mut cpu = create_cpu();
    write_op(&mut cpu, &[0xE320]); // ASL.B D1, D0 (D1 = count)
    cpu.d[0] = 0xFF;
