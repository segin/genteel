...
fn test_moveq_positive_values() {
    for data in 0..=127u8 {
        ...
        memory.write_word(0x100, 0x7000 | (data as u16)); // MOVEQ #data, D0
...
fn test_lea_all_address_registers() {
    for dst_reg in 0..8u8 {
        ...
        memory.write_word(0x100, opcode);
        memory.write_word(0x102, 0x1234);
        cpu.step_instruction(&mut memory);
...
fn test_add_overflow_detection() {
    let (mut cpu, mut memory) = create_test_cpu();
    memory.write_word(0x100, 0xD041); // ADD.W D1, D0
...
fn test_sub_borrow_detection() {
    let (mut cpu, mut memory) = create_test_cpu();
    memory.write_word(0x100, 0x9041); // SUB.W D1, D0
    cpu.d[0] = 0x0000;
    cpu.d[1] = 0x0001;
    cpu.step_instruction(&mut memory);
...
fn test_eor_all_combinations() {
    ...
    for (a, b, expected) in test_values {
        let (mut cpu, mut memory) = create_test_cpu();
        // EOR.L D0, D1 = 0xB181
        memory.write_word(0x100, 0xB181);
        cpu.d[0] = a;
        cpu.d[1] = b;
        cpu.step_instruction(&mut memory);
...
fn test_not_all_sizes() {
    // NOT.B D0
    let (mut cpu, mut memory) = create_test_cpu();
    memory.write_word(0x100, 0x4600);
    cpu.d[0] = 0x000000AA;
    cpu.step_instruction(&mut memory);
...
fn test_bra_forward() {
    for offset in [2i8, 4, 6, 8, 10, 20, 50, 100] {
        let (mut cpu, mut memory) = create_test_cpu();
        memory.write_word(0x100, 0x6000 | (offset as u8 as u16));
        cpu.step_instruction(&mut memory);
...
fn test_bcc_all_conditions() {
    // BHI (C=0 and Z=0)
    let (mut cpu, mut memory) = create_test_cpu();
    memory.write_word(0x100, 0x6204); // BHI +4
...
fn test_btst_all_bits() {
    for bit in 0..32u8 {
        let (mut cpu, mut memory) = create_test_cpu();
        memory.write_word(0x100, 0x0800); // BTST #bit, D0
...
fn test_bset_all_bits() {
    for bit in 0..32u8 {
        let (mut cpu, mut memory) = create_test_cpu();
        memory.write_word(0x100, 0x08C0); // BSET #bit, D0
        memory.write_word(0x102, bit as u16);
        cpu.d[0] = 0;
        cpu.step_instruction(&mut memory);
...
fn test_bclr_all_bits() {
    for bit in 0..32u8 {
        let (mut cpu, mut memory) = create_test_cpu();
        memory.write_word(0x100, 0x0880); // BCLR #bit, D0
        memory.write_word(0x102, bit as u16);
        cpu.d[0] = 0xFFFFFFFF;
        cpu.step_instruction(&mut memory);
...
fn test_asr_sign_extension() {
    let (mut cpu, mut memory) = create_test_cpu();
    memory.write_word(0x100, 0xE240); // ASR.W #1, D0
...
fn test_push_pop_long_sequence() {
    ...
    assert_eq!(memory.read_long(0x1FF8), 0x22222222);
    assert_eq!(memory.read_long(0x1FFC), 0x11111111);
}
...
proptest! {
    ...
    fn prop_add_commutative(a in 0u32..0xFFFF, b in 0u32..0xFFFF) {
        let (mut cpu1, mut memory1) = create_test_cpu();
        memory1.write_word(0x100, 0xD041); // ADD.W D1, D0
        cpu1.d[0] = a;
        cpu1.d[1] = b;
    ...
    fn prop_sub_zero_identity(a in 0u32..0xFFFFFFFF) {
        let (mut cpu, mut memory) = create_test_cpu();
        memory.write_word(0x100, 0x9081); // SUB.L D1, D0
        cpu.d[0] = a;
        cpu.d[1] = 0;
        cpu.step_instruction(&mut memory);
    ...
    fn prop_eor_self_zero(a in 0u32..0xFFFFFFFF) {
        let (mut cpu, mut memory) = create_test_cpu();
        memory.write_word(0x100, 0xB181); // EOR.L D0, D1
        cpu.d[0] = a;
        cpu.d[1] = a;
        cpu.step_instruction(&mut memory);
    ...
    fn prop_neg_neg_identity(a in 0u32..0xFFFFFFFF) {
        let (mut cpu, mut memory) = create_test_cpu();
        memory.write_word(0x100, 0x4480); // NEG.L D0
        memory.write_word(0x102, 0x4480); // NEG.L D0
        cpu.d[0] = a;
        cpu.step_instruction(&mut memory);
        cpu.step_instruction(&mut memory);
        prop_assert_eq!(cpu.d[0], a);
    ...
    fn prop_swap_swap_identity(a in 0u32..0xFFFFFFFF) {
        let (mut cpu, mut memory) = create_test_cpu();
        memory.write_word(0x100, 0x4840); // SWAP D0
        memory.write_word(0x102, 0x4840); // SWAP D0
        cpu.d[0] = a;
        cpu.step_instruction(&mut memory);
        cpu.step_instruction(&mut memory);
        prop_assert_eq!(cpu.d[0], a);
    ...
    fn prop_ext_word_sign(a in 0u8..=255u8) {
        let (mut cpu, mut memory) = create_test_cpu();
        memory.write_word(0x100, 0x4880); // EXT.W D0
        cpu.d[0] = a as u32;
        cpu.step_instruction(&mut memory);
...
fn test_tst_all_sizes() {
    // TST.B
    let (mut cpu, mut memory) = create_test_cpu();
    memory.write_word(0x100, 0x4A00);
    cpu.d[0] = 0xFF;
    cpu.step_instruction(&mut memory);
    assert!(cpu.get_flag(flags::NEGATIVE));
...
fn test_cmp_all_conditions() {
    // Equal
    let (mut cpu, mut memory) = create_test_cpu();
    memory.write_word(0x100, 0xB081); // CMP.L D1, D0
    cpu.d[0] = 0x12345678;
    cpu.d[1] = 0x12345678;
    cpu.step_instruction(&mut memory);
    assert!(cpu.get_flag(flags::ZERO));
...
fn test_ext_all_sizes() {
    ...
    let (mut cpu, mut memory) = create_test_cpu();
    memory.write_word(0x100, 0x4880);
    cpu.d[0] = 0x80;
    cpu.step_instruction(&mut memory);
...
fn test_exg_all_modes() {
    // EXG Dx, Dy
    let (mut cpu, mut memory) = create_test_cpu();
    memory.write_word(0x100, 0xC141); // EXG D0, D1
    cpu.d[0] = 0xAAAAAAAA;
    cpu.d[1] = 0x55555555;
    cpu.step_instruction(&mut memory);
    assert_eq!(cpu.d[0], 0x55555555);
