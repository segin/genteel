...
pub fn exec_move(cpu: &mut Cpu, size: Size, src: AddressingMode, dst: AddressingMode) -> u32 {
    let mut cycles = 4u32;
    ...
    // Read source value
    let value = cpu.cpu_read_ea(src_ea, size);
    if cpu.pending_exception { return cycles; }
    ...
    cpu.cpu_write_ea(dst_ea, size, value);
...
pub fn exec_movea(cpu: &mut Cpu, size: Size, src: AddressingMode, dst_reg: u8) -> u32 {
    let mut cycles = 4u32;
...
pub fn exec_lea(cpu: &mut Cpu, src: AddressingMode, dst_reg: u8) -> u32 {
    let (ea, cycles) = calculate_ea(src, Size::Long, &mut cpu.d, &mut cpu.a, &mut cpu.pc, &mut cpu.memory);
...
pub fn exec_clr(cpu: &mut Cpu, size: Size, dst: AddressingMode) -> u32 {
    let (dst_ea, cycles) = calculate_ea(dst, size, &mut cpu.d, &mut cpu.a, &mut cpu.pc, &mut cpu.memory);

    cpu.cpu_write_ea(dst_ea, size, 0);
...
pub fn exec_movep(cpu: &mut Cpu, size: Size, reg: u8, an: u8, reg_to_mem: bool) -> u32 {
    ...
    match size {
        Size::Word => {
            if reg_to_mem {
                ...
                cpu.memory.write_byte(addr, (val >> 8) as u8);
                cpu.memory.write_byte(addr.wrapping_add(2), val as u8);
            } else {
                let hi = cpu.memory.read_byte(addr);
                let lo = cpu.memory.read_byte(addr.wrapping_add(2));
        ...
        Size::Long => {
            if reg_to_mem {
                ...
                cpu.memory.write_byte(addr, (val >> 24) as u8);
                cpu.memory.write_byte(addr.wrapping_add(2), (val >> 16) as u8);
                cpu.memory.write_byte(addr.wrapping_add(4), (val >> 8) as u8);
                cpu.memory.write_byte(addr.wrapping_add(6), val as u8);
            } else {
                let b3 = cpu.memory.read_byte(addr);
                let b2 = cpu.memory.read_byte(addr.wrapping_add(2));
                let b1 = cpu.memory.read_byte(addr.wrapping_add(4));
                let b0 = cpu.memory.read_byte(addr.wrapping_add(6));
...
pub fn exec_movem(cpu: &mut Cpu, size: Size, to_memory: bool, ea: AddressingMode) -> u32 {
    let mask = cpu.read_word(cpu.pc);
    cpu.pc = cpu.pc.wrapping_add(2);
    ...
    let base_addr = match ea {
        ...
        _ => {
            let (ea_result, ea_cycles) = calculate_ea(ea, size, &mut cpu.d, &mut cpu.a, &mut cpu.pc, &mut cpu.memory);
            cycles += ea_cycles;
            match ea_result {
        ...
        if is_predec {
            ...
            for i in (0..16).rev() {
                if (mask & (1 << (15 - i))) != 0 {
                    ...
                    if size == Size::Word {
                        cpu.write_word(addr, val as u16);
                    } else {
                        cpu.write_long(addr, val);
                    }
            ...
            if (mask & (1 << i)) != 0 {
                if i < 8 {
                    ...
                    if size == Size::Word {
                        ...
                        cpu.d[i] = cpu.read_long(addr);
                    }
                } else {
                    // Address register: Word load is sign-extended, Long load is normal
                    if size == Size::Word {
                        cpu.a[i - 8] = cpu.read_word(addr) as i16 as i32 as u32;
                    } else {
                        cpu.a[i - 8] = cpu.read_long(addr);
                    }
                }
                addr = addr.wrapping_add(reg_size);
                cycles += if size == Size::Word { 4 } else { 8 };
...
pub fn exec_pea(cpu: &mut Cpu, src: AddressingMode) -> u32 {
    let mut cycles = 12u32;
    let (src_ea, src_cycles) = calculate_ea(src, Size::Long, &mut cpu.d, &mut cpu.a, &mut cpu.pc, &mut cpu.memory);
    cycles += src_cycles;
