...
pub fn exec_bcc<M: MemoryInterface>(cpu: &mut Cpu, condition: Condition, displacement: i16, memory: &mut M) -> u32 {
...
pub fn exec_scc<M: MemoryInterface>(cpu: &mut Cpu, condition: Condition, dst: AddressingMode, memory: &mut M) -> u32 {
    let mut cycles = 4u32;
...
pub fn exec_dbcc<M: MemoryInterface>(cpu: &mut Cpu, condition: Condition, reg: u8, memory: &mut M) -> u32 {
    if cpu.test_condition(condition) {
...
pub fn exec_link<M: MemoryInterface>(cpu: &mut Cpu, reg: u8, displacement: i16, memory: &mut M) -> u32 {
...
pub fn exec_move_usp<M: MemoryInterface>(cpu: &mut Cpu, reg: u8, to_usp: bool, memory: &mut M) -> u32 {
    if (cpu.sr & 0x2000) == 0 {
        return cpu.process_exception(8, memory); // Privilege violation
    }
    if to_usp {
...
pub fn exec_move_to_sr<M: MemoryInterface>(cpu: &mut Cpu, src: AddressingMode, memory: &mut M) -> u32 {
    if (cpu.sr & 0x2000) == 0 {
        return cpu.process_exception(8, memory); // Privilege violation
    }
...
pub fn exec_move_from_sr<M: MemoryInterface>(cpu: &mut Cpu, dst: AddressingMode, memory: &mut M) -> u32 {
...
pub fn exec_move_to_ccr<M: MemoryInterface>(cpu: &mut Cpu, src: AddressingMode, memory: &mut M) -> u32 {
    let mut cycles = 12u32;
    let (src_ea, src_cycles) = calculate_ea(src, Size::Word, &mut cpu.d, &mut cpu.a, &mut cpu.pc, memory);
    cycles += src_cycles;
