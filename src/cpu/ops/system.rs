...
pub fn exec_bra(cpu: &mut Cpu, displacement: i16, memory: &mut dyn MemoryInterface) -> u32 {
    if displacement == 0 {
...
pub fn exec_bsr(cpu: &mut Cpu, displacement: i16, memory: &mut dyn MemoryInterface) -> u32 {
...
pub fn exec_bcc(cpu: &mut Cpu, condition: Condition, displacement: i16, memory: &mut dyn MemoryInterface) -> u32 {
...
pub fn exec_scc(cpu: &mut Cpu, condition: Condition, dst: AddressingMode, memory: &mut dyn MemoryInterface) -> u32 {
    let mut cycles = 4u32;
    let (dst_ea, dst_cycles) = calculate_ea(dst, Size::Byte, &mut cpu.d, &mut cpu.a, &mut cpu.pc, memory);
    cycles += dst_cycles;

    let val = if cpu.test_condition(condition) { 0xFF } else { 0x00 };
    cpu.cpu_write_ea(dst_ea, Size::Byte, val, memory);

    cycles + if matches!(dst, AddressingMode::DataRegister(_)) { 0 } else { 4 }
}

pub fn exec_dbcc(cpu: &mut Cpu, condition: Condition, reg: u8, memory: &mut dyn MemoryInterface) -> u32 {
    if cpu.test_condition(condition) {
...
pub fn exec_jmp(cpu: &mut Cpu, dst: AddressingMode, memory: &mut dyn MemoryInterface) -> u32 {
...
pub fn exec_jsr(cpu: &mut Cpu, dst: AddressingMode, memory: &mut dyn MemoryInterface) -> u32 {
    let (ea, cycles) = calculate_ea(dst, Size::Long, &mut cpu.d, &mut cpu.a, &mut cpu.pc, memory);
...
pub fn exec_rts(cpu: &mut Cpu, memory: &mut dyn MemoryInterface) -> u32 {
    cpu.pc = memory.read_long(cpu.a[7]);
    cpu.a[7] = cpu.a[7].wrapping_add(4);
    16
}

pub fn exec_link(cpu: &mut Cpu, reg: u8, displacement: i16, memory: &mut dyn MemoryInterface) -> u32 {
...
pub fn exec_unlk(cpu: &mut Cpu, reg: u8, memory: &mut dyn MemoryInterface) -> u32 {
...
pub fn exec_trap(cpu: &mut Cpu, vector: u8, memory: &mut dyn MemoryInterface) -> u32 {
    // TRAP #n uses vectors 32-47 (0x20-0x2F).
    cpu.process_exception(32 + vector as u32, memory)
}

pub fn exec_rte(cpu: &mut Cpu, memory: &mut dyn MemoryInterface) -> u32 {
    if (cpu.sr & 0x2000) == 0 {
        // Not supervisor
...
pub fn exec_stop(cpu: &mut Cpu, memory: &mut dyn MemoryInterface) -> u32 {
    if (cpu.sr & 0x2000) == 0 {
...
pub fn exec_move_usp(cpu: &mut Cpu, reg: u8, to_usp: bool, memory: &mut dyn MemoryInterface) -> u32 {
    if (cpu.sr & 0x2000) == 0 {
        return cpu.process_exception(8, memory); // Privilege violation
    }
    if to_usp {
...
pub fn exec_rtr(cpu: &mut Cpu, memory: &mut dyn MemoryInterface) -> u32 {
...
pub fn exec_move_to_sr(cpu: &mut Cpu, src: AddressingMode, memory: &mut dyn MemoryInterface) -> u32 {
    if (cpu.sr & 0x2000) == 0 {
        return cpu.process_exception(8, memory); // Privilege violation
    }
...
pub fn exec_move_from_sr(cpu: &mut Cpu, dst: AddressingMode, memory: &mut dyn MemoryInterface) -> u32 {
    // On 68000, this is not privileged. On 68010+, it is.
    let mut cycles = 6u32;
    let (dst_ea, dst_cycles) = calculate_ea(dst, Size::Word, &mut cpu.d, &mut cpu.a, &mut cpu.pc, memory);
    cycles += dst_cycles;
...
pub fn exec_move_to_ccr(cpu: &mut Cpu, src: AddressingMode, memory: &mut dyn MemoryInterface) -> u32 {
    let mut cycles = 12u32;
    let (src_ea, src_cycles) = calculate_ea(src, Size::Word, &mut cpu.d, &mut cpu.a, &mut cpu.pc, memory);
    cycles += src_cycles;
...
pub fn exec_andi_to_ccr(cpu: &mut Cpu, memory: &mut dyn MemoryInterface) -> u32 {
...
pub fn exec_andi_to_sr(cpu: &mut Cpu, memory: &mut dyn MemoryInterface) -> u32 {
    if (cpu.sr & 0x2000) == 0 {
        return cpu.process_exception(8, memory);
    }
    let imm = memory.read_word(cpu.pc);
...
pub fn exec_ori_to_ccr(cpu: &mut Cpu, memory: &mut dyn MemoryInterface) -> u32 {
    let imm = memory.read_word(cpu.pc) & 0x00FF;
    cpu.pc = cpu.pc.wrapping_add(2);
    cpu.sr = (cpu.sr & 0xFF00) | ((cpu.sr | imm) & 0x00FF);
    20
...
pub fn exec_ori_to_sr(cpu: &mut Cpu, memory: &mut dyn MemoryInterface) -> u32 {
    if (cpu.sr & 0x2000) == 0 {
        return cpu.process_exception(8, memory);
    }
    let imm = memory.read_word(cpu.pc);
...
pub fn exec_eori_to_ccr(cpu: &mut Cpu, memory: &mut dyn MemoryInterface) -> u32 {
    let imm = memory.read_word(cpu.pc) & 0x00FF;
    cpu.pc = cpu.pc.wrapping_add(2);
    cpu.sr = (cpu.sr & 0xFF00) | ((cpu.sr ^ imm) & 0x00FF);
    20
...
pub fn exec_eori_to_sr(cpu: &mut Cpu, memory: &mut dyn MemoryInterface) -> u32 {
    if (cpu.sr & 0x2000) == 0 {
        return cpu.process_exception(8, memory);
    }
    let imm = memory.read_word(cpu.pc);
