...
fn test_btst_immediate() {
    let mut cpu = create_cpu();
    write_op(&mut cpu, &[0x0800, 0x0007]); // BTST #7, D0
...
fn test_btst_all_32_bits() {
    for bit in 0..32u8 {
        let mut cpu = create_cpu();
        write_op(&mut cpu, &[0x0800, bit as u16]); // BTST #bit, D0
        cpu.d[0] = 1u32 << bit;
        cpu.pc = 0x1000;
        cpu.step_instruction();
...
fn test_bset_register() {
    let mut cpu = create_cpu();
    write_op(&mut cpu, &[0x01C0]); // BSET D0, D0 (uses D0 as bit number)
    cpu.d[0] = 4; // Set bit 4
    cpu.step_instruction();
...
fn test_bset_immediate() {
    let mut cpu = create_cpu();
    write_op(&mut cpu, &[0x08C0, 0x0003]); // BSET #3, D0
    cpu.d[0] = 0x00;
    cpu.step_instruction();
...
fn test_bclr_immediate() {
    let mut cpu = create_cpu();
    write_op(&mut cpu, &[0x0880, 0x0003]); // BCLR #3, D0
    cpu.d[0] = 0xFF;
    cpu.step_instruction();
...
fn test_bchg_set_to_clear() {
    let mut cpu = create_cpu();
    write_op(&mut cpu, &[0x0840, 0x0003]); // BCHG #3, D0
...
fn test_move_b_d0_d1() {
    let mut cpu = create_cpu();
    write_op(&mut cpu, &[0x1200]); // MOVE.B D0, D1
    cpu.d[0] = 0x42;
    cpu.d[1] = 0xFF00FF00;
    cpu.step_instruction();
...
fn test_move_w_d0_d1() {
    let mut cpu = create_cpu();
    write_op(&mut cpu, &[0x3200]); // MOVE.W D0, D1
    cpu.d[0] = 0x1234;
    cpu.d[1] = 0xFFFF0000;
    cpu.step_instruction();
...
fn test_move_l_d0_d1() {
    let mut cpu = create_cpu();
    write_op(&mut cpu, &[0x2200]); // MOVE.L D0, D1
    cpu.d[0] = 0x12345678;
    cpu.d[1] = 0xFFFFFFFF;
    cpu.step_instruction();
...
fn test_movea_w() {
    let mut cpu = create_cpu();
    write_op(&mut cpu, &[0x3040]); // MOVEA.W D0, A0
...
fn test_movea_l() {
    let mut cpu = create_cpu();
    write_op(&mut cpu, &[0x2040]); // MOVEA.L D0, A0
    cpu.d[0] = 0x12345678;
    cpu.step_instruction();
...
fn test_moveq_positive() {
    let mut cpu = create_cpu();
    write_op(&mut cpu, &[0x7042]); // MOVEQ #$42, D0
    cpu.step_instruction();
...
fn test_moveq_negative() {
    let mut cpu = create_cpu();
    write_op(&mut cpu, &[0x70FF]); // MOVEQ #-1, D0
    cpu.step_instruction();
...
fn test_moveq_zero() {
    let mut cpu = create_cpu();
    write_op(&mut cpu, &[0x7000]); // MOVEQ #0, D0
    cpu.step_instruction();
    assert_eq!(cpu.d[0], 0);
    assert!(cpu.get_flag(flags::ZERO));
}
...
fn test_exg_data_data() {
    let mut cpu = create_cpu();
    write_op(&mut cpu, &[0xC141]); // EXG D0, D1 (data-data, mode 01000)
    cpu.d[0] = 0x11111111;
    cpu.d[1] = 0x22222222;
    cpu.step_instruction();
...
fn test_exg_addr_addr() {
    let mut cpu = create_cpu();
    write_op(&mut cpu, &[0xC149]); // EXG A0, A1 (addr-addr, mode 01001)
    cpu.a[0] = 0x11111111;
    cpu.a[1] = 0x22222222;
    cpu.step_instruction();
...
fn test_exg_data_addr() {
    let mut cpu = create_cpu();
    write_op(&mut cpu, &[0xC188]); // EXG D0, A0 (data-addr, mode 10001)
    cpu.d[0] = 0x11111111;
    cpu.a[0] = 0x22222222;
    cpu.step_instruction();
    assert_eq!(cpu.d[0], 0x22222222);
...
fn test_swap_d0() {
    let mut cpu = create_cpu();
    write_op(&mut cpu, &[0x4840]); // SWAP D0
    cpu.d[0] = 0x12345678;
    cpu.step_instruction();
...
fn test_ext_w_positive() {
    let mut cpu = create_cpu();
    write_op(&mut cpu, &[0x4880]); // EXT.W D0
...
fn test_ext_l_positive() {
    let mut cpu = create_cpu();
    write_op(&mut cpu, &[0x48C0]); // EXT.L D0
    cpu.d[0] = 0x00007FFF;
    cpu.step_instruction();
...
fn test_clr_b() {
    let mut cpu = create_cpu();
    write_op(&mut cpu, &[0x4200]); // CLR.B D0
    cpu.d[0] = 0xFFFFFFFF;
    cpu.step_instruction();
...
fn test_clr_w() {
    let mut cpu = create_cpu();
    write_op(&mut cpu, &[0x4240]); // CLR.W D0
    cpu.d[0] = 0xFFFFFFFF;
    cpu.step_instruction();
...
fn test_clr_l() {
    let mut cpu = create_cpu();
    write_op(&mut cpu, &[0x4280]); // CLR.L D0
    cpu.d[0] = 0xFFFFFFFF;
    cpu.step_instruction();
...
fn test_movem_to_memory() {
    let mut cpu = create_cpu();
    // MOVEM.L D0-D3, (A0)
    write_op(&mut cpu, &[0x48D0, 0x000F]);
    cpu.a[0] = 0x2000;
    cpu.d[0] = 0x11111111;
    cpu.d[1] = 0x22222222;
    cpu.d[2] = 0x33333333;
    ...
    assert_eq!(cpu.memory.read_long(0x2008), 0x33333333);
    assert_eq!(cpu.memory.read_long(0x200C), 0x44444444);
}
...
fn test_movem_from_memory() {
    let mut cpu = create_cpu();
    // MOVEM.L (A0), D0-D3
    write_op(&mut cpu, &[0x4CD0, 0x000F]);
    cpu.a[0] = 0x2000;
    cpu.memory.write_long(0x2000, 0xAAAAAAAA);
    cpu.memory.write_long(0x2004, 0xBBBBBBBB);
    cpu.memory.write_long(0x2008, 0xCCCCCCCC);
    cpu.memory.write_long(0x200C, 0xDDDDDDDD);
    cpu.step_instruction();
...
fn test_lea_absolute() {
    let mut cpu = create_cpu();
    // LEA $1234.W, A0
    write_op(&mut cpu, &[0x41F8, 0x1234]);
    cpu.step_instruction();
...
fn test_lea_displacement() {
    let mut cpu = create_cpu();
    // LEA (d16, A1), A0
    write_op(&mut cpu, &[0x41E9, 0x0100]);
    cpu.a[1] = 0x2000;
    cpu.step_instruction();
...
fn test_pea() {
    let mut cpu = create_cpu();
    // PEA $1234.W
    write_op(&mut cpu, &[0x4878, 0x1234]);
    cpu.a[7] = 0x8000;
    cpu.step_instruction();
    assert_eq!(cpu.a[7], 0x7FFC);
    assert_eq!(cpu.memory.read_long(0x7FFC), 0x1234);
}
