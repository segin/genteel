...
impl Vdp {
    ...
    fn render_tile(&mut self, screen_x: u16, config: &PlaneRenderConfig) -> u16 {
        ...
        let (v_scroll, _) = self.get_scroll_values(
            config.is_plane_a,
            config.fetch_line,
            (screen_x >> 3) as usize,
        );
        ...
        let entry = self.fetch_nametable_entry(
            config.name_table_base,
            tile_v,
            tile_h,
            config.plane_w,
        );
        ...
        let pixels_to_process =
            std::cmp::min(pixels_left_in_tile, config.screen_width - screen_x);
