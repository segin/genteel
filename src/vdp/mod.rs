    ...
    pub fn write_data(&mut self, value: u16) {
        ...
        match self.control_code & 0x0F {
            ...
            3 => { // Write CRAM
                ...
                let r = (value & 0xE) << 1;  // 3 bits -> 4 bits
                let g = (value & 0xE0) >> 3; // 3 bits -> 4 bits
                let b = (value & 0xE00) >> 7;// 3 bits -> 4 bits
                // Expand to 5/6/5
                let r5 = (r << 1) | (r >> 3);
                let g6 = (g << 2) | (g >> 2);
                let b5 = (b << 1) | (b >> 3);
                ...
                self.cram[addr] = (value & 0xFF) as u8;
                self.cram[addr + 1] = (value >> 8) as u8;
            },
            5 => { // Write VSRAM
                let addr = (self.control_address & 0x7F) as usize;
                if addr < 80 {
