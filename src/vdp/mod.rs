...
impl Vdp {
    ...
    fn get_plane_scroll(&self, is_plane_a: bool) -> (u16, u16) {
        // Get vertical scroll
    ...
    fn fetch_nametable_entry(
        ...
        let nt_entry_addr = base_addr + (row * width + col) * 2;
        let hi = self.vram[nt_entry_addr & 0xFFFF];
        let lo = self.vram[(nt_entry_addr + 1) & 0xFFFF];
        ((hi as u16) << 8) | (lo as u16)
    }

    fn fetch_pattern_row(&self, tile_index: u16, row: u16) -> [u8; 4] {
        let row_addr = (tile_index as usize * 32) + (row as usize * 4);
        [
            self.vram[row_addr & 0xFFFF],
            self.vram[(row_addr + 1) & 0xFFFF],
            self.vram[(row_addr + 2) & 0xFFFF],
            self.vram[(row_addr + 3) & 0xFFFF],
        ]
    }

    fn draw_plane_pixels(
        &mut self,
        screen_x: u16,
        count: u16,
        pixel_h: u16,
    ...
    fn render_plane(
        ...
        let (v_scroll, h_scroll) = self.get_plane_scroll(is_plane_a);
        ...
        while screen_x < screen_width {
            ...
            let entry = self.fetch_nametable_entry(name_table_base, plane_w, tile_h, tile_v);
            ...
            let patterns = self.fetch_pattern_row(tile_index, row);

            self.draw_plane_pixels(
                screen_x,
                pixels_to_process,
                pixel_h,
                &patterns,
