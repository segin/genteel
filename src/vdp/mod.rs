...
impl Vdp {
    ...
    unsafe fn draw_full_tile_row(&mut self, entry: u16, pixel_v: u16, dest_idx: usize) {
        ...
        // Optimization: Early exit if the whole row is transparent (all zeros)
        if u32::from_ne_bytes(patterns) == 0 {
            return;
        }

        let (p0, p1, p2, p3) = if h_flip {
            // Swap nibbles to align with !h_flip order: (lo, hi) -> (hi, lo) for processing
            // Also swap byte order: 0<->3, 1<->2
            (
                (patterns[3] >> 4) | (patterns[3] << 4),
                (patterns[2] >> 4) | (patterns[2] << 4),
                (patterns[1] >> 4) | (patterns[1] << 4),
                (patterns[0] >> 4) | (patterns[0] << 4),
            )
        } else {
