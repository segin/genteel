        ...
        if self.gui_state.is_window_open("Tile Viewer") {
            ...
            egui::Window::new("Tile Viewer")
                ...
                .show(&self.egui_ctx, |ui| {
                    ...
                    let texture = if let Some(tex) = &mut self.tile_texture {
                        tex.set(image, Default::default());
                        tex
                    } else {
                        self.tile_texture.insert(ui.ctx().load_texture(
                            "tile_viewer",
                            image,
                            Default::default(),
                        ))
            ...
            egui::Window::new("Scroll Plane Viewer")
                ...
                .show(&self.egui_ctx, |ui| {
                    ...
                    let render_plane = |ui: &mut egui::Ui,
                        ...
                        let texture = if let Some(tex) = texture_opt {
                            tex.set(image, Default::default());
                            tex
                        } else {
                            texture_opt.insert(ui.ctx().load_texture(
                                id,
                                image,
                                Default::default(),
                            ))
